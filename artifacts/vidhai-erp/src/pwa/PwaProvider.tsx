...
function Status({state,showIos,closeIos,later}:{state:PwaState;showIos:boolean;closeIos():void;later():void}){
  ...
  if(state.installAvailable||state.iosInstallAvailable)return <button className="fixed z-[90] right-[max(1rem,env(safe-area-inset-right))] bottom-[max(1rem,env(safe-area-inset-bottom))] hidden rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground shadow-lg sm:block" onClick={()=>void state.install()}>Install App</button>;
  return null;
}
