    ...
    <div className="space-y-4">
      ...
      {loading ? <div className="flex min-h-56 items-center justify-center rounded-lg border bg-white text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading financial statements...</div> : (
        <Tabs value={report} onValueChange={setReport}>
          ...
          <TabsContent value="trial-balance"><div className="overflow-x-auto max-h-[calc(100vh-280px)] overflow-y-auto rounded-lg border bg-white"><table className="w-full text-sm whitespace-nowrap"><thead className="sticky top-0 z-20 bg-slate-100 dark:bg-muted font-semibold shadow-sm"><tr><th className="sticky top-0 z-20 bg-slate-100 dark:bg-muted px-4 py-3 text-left whitespace-nowrap border-b">Account Code</th><th className="sticky top-0 z-20 bg-slate-100 dark:bg-muted px-4 py-3 text-left whitespace-nowrap border-b">Account Name</th><th className="sticky top-0 z-20 bg-slate-100 dark:bg-muted px-4 py-3 text-left whitespace-nowrap border-b">Type</th><th className="sticky top-0 z-20 bg-slate-100 dark:bg-muted px-4 py-3 text-right whitespace-nowrap border-b">Debit</th><th className="sticky top-0 z-20 bg-slate-100 dark:bg-muted px-4 py-3 text-right whitespace-nowrap border-b">Credit</th></tr></thead><tbody className="divide-y">{calculated.trial.map((account) => <tr key={account.id}><td className="px-4 py-2.5 font-mono whitespace-nowrap">{account.accountCode}</td><td className="px-4 py-2.5 whitespace-nowrap">{account.accountName}</td><td className="px-4 py-2.5 whitespace-nowrap">{account.accountType}</td><td className="px-4 py-2.5 text-right tabular-nums whitespace-nowrap">{money(account.trialDebit)}</td><td className="px-4 py-2.5 text-right tabular-nums whitespace-nowrap">{money(account.trialCredit)}</td></tr>)}</tbody><tfoot className="border-t-2 bg-muted/30 font-semibold whitespace-nowrap"><tr><td className="px-4 py-3 whitespace-nowrap" colSpan={3}>Total</td><td className="px-4 py-3 text-right whitespace-nowrap">{money(calculated.totalDebit)}</td><td className="px-4 py-3 text-right whitespace-nowrap">{money(calculated.totalCredit)}</td></tr><tr><td className={Math.abs(calculated.trialDifference) <= 0.01 ? "px-4 py-3 text-emerald-700 whitespace-nowrap" : "px-4 py-3 text-red-600 whitespace-nowrap"} colSpan={5}>Difference: {money(calculated.trialDifference)}</td></tr></tfoot></table></div></TabsContent>
        </Tabs>
      )}
    </div>
  );
