...
const useMemberForm = () => {
  ...
  if (!value) throw new Error("Member form fields must be inside AddMemberDialog");
  return value;
};
...
export function AddMemberDialog({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([code, data]) => {
        setF((x) => ({ ...x, employeeCode: code.employeeCode }));
        setOptions(data);
        setF((x) => ({
          ...x,
          ...
          <div className="flex-1 space-y-7 overflow-y-auto px-6 py-6">
            <Section title="Profile">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {can("settings.user_management.create") && (
            ...
            <Section title="HR & Compliance">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Text
                  ...
                  min="0"
                />
                <Text k="joinDate" label="Joining Date" required type="date" />
                <Text
                  k="exitDate"
                  label="Exit Date"
                  type="date"
...
function Text({
  ...
  return <F label={label} required={required} error={errors[k]} name={k} refs={refs}>
    <Input className={errors[k] ? "border-destructive" : ""} value={String(f[k] ?? "")} onChange={(e) => field(k, e.target.value)} {...props}/>
  </F>;
}
function Choice({
  k,
  ...
  const { f, errors, refs, field } = useMemberForm();
  return <F label={label} required={required} error={errors[k]} name={k} refs={refs}>
    <select className={`h-10 w-full rounded-md border border-input bg-background px-3 text-sm shadow-xs outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 ${errors[k] ? "border-destructive" : ""}`} value={String(f[k] ?? "")} onChange={(e) => field(k, e.target.value)}>
      <option value="">Select {label.toLowerCase()}</option>
      {items.map((item:any)=>{const value=typeof item==="string"?item:item.value,label=typeof item==="string"?item:item.label;return <option key={value} value={value} disabled={Boolean(typeof item==="object"&&item.disabled)}>{label}</option>})}
    </select>
  </F>;
}
function Template({
  ...
  const { f } = useMemberForm(); const joiningYear=Number(String(f.joinDate||today()).slice(0,4)); const holiday=k==="holidayTemplate"; return <Choice k={k} label={label} required={required} items={items.map((item:any)=>{const wrongYear=holiday&&Number(item.effectiveYear)!==joiningYear;return {value:String(item.id),disabled:wrongYear,label:`${item.templateName}${holiday&&item.effectiveYear?` (${item.effectiveYear})`:""}${item.isDefault?" (Default)":""}${wrongYear?" � Not applicable to joining year":""}`}})}/>;
}
