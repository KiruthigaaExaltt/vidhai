...
export default function BatchDetail() {
  ...
  useEffect(() => {
    ...
    // Batch and chamber queries refresh independently after T4. Keep the fallback prompt
    // synchronized so a transient stale chamber response cannot leave it open.
    setChamberPromptOpen(!assignedBulkChamber);
  ...
  const assignCurrentBulkChamber = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(body.error || "Failed to assign chamber");
      await queryClient.invalidateQueries({
        queryKey: getListChambersQueryKey(),
      });
      setChamberPromptOpen(false);
      toast.success("Bulk chamber assigned");
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to assign chamber",
      );
    } finally {
      setAssigningChamber(false);
    }
  };
  ...
  const openCompleteDialog = (stageKey: string) => {
    ...
    const nextStageKey =
      idx < STAGE_SEQ.length - 1 ? STAGE_SEQ[idx + 1].key : "COMPLETED";
    setStageImages([null, null]);
    setCompleteDialog({
      open: true,
      stageKey,
  ...
  const nPassed =
    formulationStats.nPercent >= 1.5 && formulationStats.nPercent <= 1.8;
  ...
  if (!batch)
    return (
      <Shell>
        <div className="p-8 text-destructive text-sm">Batch not found</div>
      </Shell>
    );
        ...
        <div className="flex flex-col md:flex-row md:items-start gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Location: {batch.locationCode} · Created:{" "}
              ...
              {batch.targetBags && (
                <>
                  {" "}
                  · Target:{" "}
                  <strong>{batch.targetBags.toLocaleString()} bags</strong>
                </>
              )}
            </p>
          </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      This is a permanent record of what was formulated when the
                      batch was initiated. It cannot be changed.
                    </p>
                  </div>
                  {formulationStats.items.length > 0 && (
                    <div className="flex items-center gap-3 bg-background border px-4 py-2 rounded-sm shadow-sm shrink-0">
                      ...
                      <div
                        className={`font-mono text-2xl font-bold ${nPassed ? "text-green-600" : "text-destructive"}`}
                      >
                        {formulationStats.nPercent.toFixed(2)}%
                      </div>
                      <Badge
                        variant="outline"
                        className={`border-0 rounded-sm uppercase tracking-wider px-2 py-0.5 text-[10px] font-semibold ${nPassed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                        {nPassed ? "PASS" : "FAIL"}
                      </Badge>
                    </div>
                      ...
                      <table className="w-full text-sm text-left whitespace-nowrap">
                        <thead className="bg-muted/30 text-muted-foreground text-[10px] uppercase tracking-wider border-b border-border">
                          <tr>
                            ...
                            <th className="px-4 py-3 font-medium text-right bg-blue-50/40">
                              Wet Weight (kg)
                            </th>
                            <th className="px-4 py-3 font-medium text-right bg-blue-50/40">
                              H₂O %
                            </th>
                            <th className="px-4 py-3 font-medium text-right bg-primary/5 text-primary/70">
                              Dry Weight (kg)
                            </th>
                            <th className="px-4 py-3 font-medium text-right bg-emerald-50/40">
                              N %
                            </th>
                            <th className="px-4 py-3 font-medium text-right bg-primary/5 text-primary/70">
                              N₂ (kg)
                        ...
                        <tbody className="divide-y divide-border">
                          {formulationStats.items.map((m: any, idx: number) => (
                            <tr
                              key={idx}
                              className="h-[42px] hover:bg-muted/10"
                            >
                              <td className="px-4 font-medium">
                                {m.materialName}
                              </td>
                              <td className="px-4 font-mono text-right bg-blue-50/10">
                                {m.wet.toFixed(3)}
                              </td>
                              <td className="px-4 font-mono text-right bg-blue-50/10">
                                {Number(m.moisturePercent).toFixed(1)}
                              </td>
                              <td className="px-4 font-mono text-right font-bold text-primary bg-primary/5">
                                {m.dry.toFixed(3)}
                              </td>
                              <td className="px-4 font-mono text-right bg-emerald-50/10">
                                {Number(m.nitrogenPercent).toFixed(2)}
                              </td>
                              <td className="px-4 font-mono text-right font-bold text-primary bg-primary/5">
                                {m.n2.toFixed(4)}
                              </td>
                        ...
                        <tfoot className="border-t-2 border-border bg-muted/40">
                          <tr className="h-[42px] text-sm font-semibold">
                            <td className="px-4 text-[10px] uppercase tracking-wider text-muted-foreground">
                              Totals
                            </td>
                            <td className="px-4 font-mono text-right font-bold">
                              {formulationStats.totalWet.toFixed(2)}
                            </td>
                            <td className="px-4 font-mono text-right text-muted-foreground">
                              —
                            </td>
                            <td className="px-4 font-mono text-right font-bold text-primary bg-primary/5">
                              {formulationStats.totalDry.toFixed(2)}
                            </td>
                            <td className="px-4 font-mono text-right text-muted-foreground">
                              —
                            </td>
                            <td className="px-4 font-mono text-right font-bold text-primary bg-primary/5">
                              {formulationStats.totalN2.toFixed(4)}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
                      ...
                      <div className="px-5 py-3 border-t bg-muted/10 flex items-center gap-6 text-xs text-muted-foreground">
                        <span>
                          ...
                          <strong className="text-foreground font-mono">
                            {batch.targetBags.toLocaleString()}
                          </strong>
                        </span>
                        <span>
                          Total wet:{" "}
                          <strong className="text-foreground font-mono">
                            {formulationStats.totalWet.toFixed(1)} kg
                          </strong>
                        </span>
                        <span>
                          Total dry:{" "}
                          <strong className="text-foreground font-mono">
                            {formulationStats.totalDry.toFixed(1)} kg
                          </strong>
                        </span>
                      </div>
                    )}
                    ...
                    {STAGE_SEQ.map((stage, idx) => {
                      ...
                      const expectedEnd =
                        stage.duration > 0
                          ...
                          : null;
                      const logEntry = batch.stageLogs?.find(
                        (l: any) => l.stage === stage.key,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <p
                              className={`text-sm font-semibold leading-tight mb-2 ${isPending ? "text-muted-foreground" : "text-foreground"}`}
                            >
                              {stage.label}
                            </p>
                            ...
                            <div className="text-[11px] text-muted-foreground space-y-0.5">
                              <p>
                                Expected: {fmtDate(expectedStart)}
                                {expectedEnd
                                  ? ` – ${fmtDate(expectedEnd)}`
                                  : ""}
                              </p>
                            ...
                            {isActive &&
                              ...
                              (stage.key === "DISPATCH" ||
                                (batch.status as string) !== "dispatched") && (
                                <Button
                                  size="sm"
                            ...
                            {isCompleted &&
                              (logEntry as any)?.verificationImages?.length >
                                0 && (
                                <div className="flex gap-1 mt-2">
                                  {(logEntry as any).verificationImages
                                    .slice(0, 2)
      ...
      <Dialog open={chamberPromptOpen} onOpenChange={setChamberPromptOpen}>
        <DialogContent className="rounded-sm border-border max-w-md shadow-xl">
          <DialogHeader>
            ...
            <p className="text-sm text-muted-foreground">
              This batch is in the Bulk Chamber stage but has no chamber
              assigned. Select an available chamber to continue.
            </p>
          </DialogHeader>
          <div className="space-y-2 py-2">
            <Label className="text-xs uppercase tracking-wider text-muted-foreground">
              Available Bulk Chambers
            </Label>
            <Select value={promptChamberId} onValueChange={setPromptChamberId}>
              <SelectTrigger className="rounded-sm">
                <SelectValue placeholder="Select a bulk chamber..." />
              </SelectTrigger>
              <SelectContent>
                {availableBulkChambers.map((chamber: any) => (
                  <SelectItem key={chamber.id} value={String(chamber.id)}>
                    {chamber.name}
                    {chamber.capacity ? ` - ${chamber.capacity} bags` : ""}
                  </SelectItem>
                ))}
                {availableBulkChambers.length === 0 && (
                  <SelectItem value="__none__" disabled>
                    No bulk chambers available
                  </SelectItem>
                )}
              </SelectContent>
            </Select>
            {availableBulkChambers.length === 0 && (
              <p className="text-xs text-amber-600">
                Create a Bulk chamber or make an occupied chamber available
                first.
              </p>
            )}
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setChamberPromptOpen(false)}
            >
              Later
            </Button>
            <Button
              disabled={!promptChamberId || assigningChamber}
              onClick={assignCurrentBulkChamber}
            >
              {assigningChamber ? "Assigning..." : "Assign Chamber"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className="space-y-4 pt-1">
            ...
            {isCompletingDispatch && (
              <div className="p-3 bg-muted/30 border border-border rounded-sm space-y-3">
                ...
                <Input
                  ...
                  min="1"
                  step="1"
                  value={completeDialog.producedBags}
                  onChange={(e) =>
                    setCompleteDialog((previous) => ({
                      ...previous,
                      producedBags: e.target.value,
                    }))
                  }
                ...
                {!producedBagsReady && (
                  <p className="text-xs text-amber-600 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> Enter a positive
                    whole number of produced bags.
                  </p>
                )}
              </div>
            )}
            {isEnteringBulkChamber && (
              <div className="p-3 bg-muted/30 border border-border rounded-sm space-y-3">
                ...
                >
                  <SelectTrigger className="rounded-sm h-9 bg-white">
                    <SelectValue placeholder="Select an available bulk chamber..." />
                  </SelectTrigger>
                  <SelectContent>
                    {availableBulkChambers.map((chamber: any) => (
                      <SelectItem key={chamber.id} value={String(chamber.id)}>
                ...
                {!completeDialog.chamberId && (
                  <p className="text-xs text-amber-600 flex items-center gap-1">
                    ...
                    {availableBulkChambers.length === 0
                      ? "Create a Bulk chamber or make an occupied chamber available first."
                      : "Select a Bulk chamber to continue."}
                  </p>
                )}
              </div>
            )}
              ...
              <div className="p-3 bg-muted/30 border border-border rounded-sm space-y-3">
                ...
                {completeDialog.spawnType === "internal" ? (
                  <Select
                    ...
                    onValueChange={(v) =>
                      setCompleteDialog((p) => ({ ...p, spawnRef: v }))
                    }
                  >
                    <SelectTrigger className="rounded-sm h-9 bg-white">
