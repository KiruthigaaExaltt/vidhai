...
const STAGES = [
  ...
  { key: "MOTHER_CULTURE", label: "Mother Culture", days: 17, icon: FlaskConical },
...
export default function LabBatchDetail() {
  ...
  const refetch = () => {
    queryClient.invalidateQueries({ queryKey: getGetLabBatchQueryKey(batchId) });
  ...
  const totalKg = formulationRows.reduce((s, r) => s + (parseFloat(r.qtyKg) || 0), 0);
  ...
  const handleImageFile = (slot: 0 | 1, file: File) => {
    ...
    reader.onload = e => {
      setStageImages(prev => { const n = [...prev]; n[slot] = e.target?.result as string; return n; });
    };
  ...
  const isFinalStage  = advDialog.stageKey === "SHAKING_2";
  const imagesReady   = stageImages[0] !== null && stageImages[1] !== null;
  const finalReady    = !isFinalStage || (!!advDialog.strainName && Number(advDialog.spawnQty) > 0);
  ...
  let advanceMutation = useMutation({
    ...
    onSuccess: () => {
      refetch();
      const isCompletion = advDialog.stageKey === "SHAKING_2";
      toast.success(isCompletion ? "Batch completed — spawn stock recorded" : "Stage completed");
      closeAdvDialog();
    },
  ...
  const completedStageKeys = useMemo(() => {
    return new Set(stageLogs.filter((l: any) => l.exitedAt).map((l: any) => l.stage));
  }, [stageLogs]);

  const totalDays = STAGES.reduce((s, st) => s + st.days, 0);
  const doneDays  = STAGES.slice(0, Math.max(0, curIdx)).reduce((s, st) => s + st.days, 0);
  const savedTotalKg = savedMaterials.reduce((s: number, m: any) => s + Number(m.quantityKg), 0);
  ...
  return (
    <Shell>
      <div className="min-w-0 w-full space-y-6 p-6 md:p-8">
        ...
        {/* Back */}
        <Button variant="ghost" onClick={() => setLocation("/lab/batches")}
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <Badge variant="outline" className={`border-0 rounded-sm text-[11px] uppercase tracking-wider font-semibold px-2 py-0.5 ${
                ...
                isFormulation ? "bg-muted text-muted-foreground" :
                                "bg-sky-50 text-sky-700"
              }`}>
                {isFormulation ? "Formulation" : isCompleted ? "Completed" : currentStage.replace(/_/g, " ")}
              </Badge>
            </div>
          ...
          <div className="flex flex-wrap gap-2 md:justify-end">
            <div className="px-3 py-2 rounded-sm border border-border bg-muted/30 text-sm">
              ...
              <span className="font-mono font-bold">
                {isFormulation ? totalKg.toFixed(2) : savedTotalKg.toFixed(2)} kg
              </span>
            </div>
            {!isFormulation && (
              <div className="px-3 py-2 rounded-sm border border-border bg-muted/30 text-sm">
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mr-1.5">Progress</span>
                <span className="font-mono font-bold">{doneDays} / {totalDays} days</span>
              </div>
            )}
            {spawnOutputs.length > 0 && (
              <div className="px-3 py-2 rounded-sm border border-primary/20 bg-primary/5 text-sm">
                <span className="text-[10px] uppercase tracking-wider text-primary font-semibold mr-1.5">Output</span>
                <span className="font-mono font-bold text-primary">
                  {spawnOutputs.reduce((s: number, o: any) => s + Number(o.quantityKg), 0).toFixed(1)} kg
                </span>
              </div>
            )}
          </div>
            ...
            <CardHeader className="pb-0 border-b flex flex-row items-start justify-between">
              <div className="pb-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Pre-filled with the standard reference recipe. Adjust quantities for this batch,
                  then click <strong>Initiate Batch</strong> to lock the formulation and begin the 9-stage tracker.
                </p>
              </div>
              <Button
                        ...
                        <tr key={row.id} className="h-[44px] hover:bg-muted/30">
                          ...
                          <td className="px-6 text-right">
                            <input
                              ...
                              value={row.qtyKg}
                              onChange={e => setFormulationRows(rows => rows.map(r => r.id === row.id ? { ...r, qtyKg: e.target.value } : r))}
                        ...
                        <td className="px-4 text-right">
                          <button
                            onClick={() => {
                              ...
                              setFormulationRows(rows => [...rows, { id: Date.now(), name: newMatName, qtyKg: newMatQty }]);
                  ...
                  {formulationRows.length > 0 && (
                    <tfoot className="border-t-2 border-border bg-muted/50">
                      <tr className="h-[44px] font-semibold">
                        <td className="px-6 text-xs uppercase tracking-wider">Total Input</td>
                        <td className="px-6 font-mono text-right text-primary text-base">{totalKg.toFixed(3)} kg</td>
                        <td className="px-6 font-mono text-right text-primary">100.0%</td>
                        <td />
                      </tr>
                    </tfoot>
                  )}
              ...
              <div className="p-5 border-t border-border bg-muted/10 flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Expected output: <span className="font-semibold text-primary">51–54 kg spawn</span> per standard batch.
                    Formulation is locked once initiated.
                  </p>
                </div>
                <Button
        ...
        {!isFormulation && (
          <Tabs defaultValue="tracker" className="w-full">
            <TabsList className="rounded-none bg-transparent border-b w-full justify-start h-auto p-0 space-x-6 mb-0">
              ...
              <TabsTrigger value="output" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 py-2.5 font-medium text-sm">
                History & Output
              </TabsTrigger>
            </TabsList>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Read-only snapshot of the batch formulation as recorded at initiation.
                      </p>
                    </div>
                ...
                <CardContent className="p-0">
                  {savedMaterials.length === 0 ? (
                    <div className="p-8 text-center text-sm text-muted-foreground">
                      No formulation data recorded — this batch was created before formulation tracking was introduced.
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-left whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-border">
                          {savedMaterials.map((m: any) => {
                            ...
                            const pct = savedTotalKg > 0 ? (qty / savedTotalKg) * 100 : 0;
                            return (
                              <tr key={m.id} className="h-[44px] bg-muted/5">
                                <td className="px-6 font-medium">{m.name}</td>
                                <td className="px-6 font-mono text-right font-semibold">{qty.toFixed(3)}</td>
                                <td className="px-6 font-mono text-right text-muted-foreground">{pct.toFixed(1)}%</td>
                              </tr>
                            );
                          })}
                        ...
                        <tfoot className="border-t-2 border-border bg-muted/50">
                          <tr className="h-[44px] font-bold">
                            <td className="px-6 text-xs uppercase tracking-wider">Total Input</td>
                            <td className="px-6 font-mono text-right text-primary text-base">{savedTotalKg.toFixed(3)} kg</td>
                            <td className="px-6 font-mono text-right text-primary">100.0%</td>
                          </tr>
                        </tfoot>
                      </table>
            ...
            <TabsContent value="tracker" className="pt-5 space-y-4">
              ...
              {!isCompleted && (
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span className="uppercase tracking-wider font-medium">Overall Progress</span>
                    <span className="font-mono">{doneDays} / {totalDays} planned days</span>
                  </div>
                  <div className="h-1.5 w-full bg-border rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all rounded-full"
                      style={{ width: `${Math.min(100, (doneDays / totalDays) * 100)}%` }} />
                  </div>
                </div>
              )}
              ...
              {isCompleted && (
                <div className="p-4 border border-primary/20 bg-primary/5 rounded-sm flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-primary">All Stages Complete — Spawn Stock Produced</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Check the History &amp; Output tab to see the produced spawn stock and its destination.
                    </p>
                  </div>
                </div>
              )}
              ...
              <Card className="rounded-sm border-border shadow-none">
                <CardHeader className="pb-3 border-b">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Complete each stage in order. Two verification photos required per stage.
                  </p>
                </CardHeader>
                <CardContent className="p-4 space-y-2">
                  {STAGES.map((stage, idx) => {
                    const isDone   = isCompleted || completedStageKeys.has(stage.key);
                    const isActive = !isCompleted && !isDone && stage.key === currentStage;
                    const isPending = !isDone && !isActive;
                    const log = stageLogs.find((l: any) => l.stage === stage.key) ?? null;
                    const photos: string[] = log?.verificationImages ?? [];
                    const Icon = stage.icon;

                    return (
                      <div key={stage.key} className={`flex items-start gap-4 p-3 rounded-sm border-2 transition-all ${
                        ...
                        <div className="flex flex-col items-center gap-1 shrink-0">
                          <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 ${
                            ...
                            {isDone ? <CheckCircle2 className="w-4 h-4" /> : isActive ? <Clock className="w-4 h-4 animate-pulse" /> : <Icon className="w-4 h-4" />}
                          </div>
                          {idx < STAGES.length - 1 && (
                            <div className={`w-0.5 h-3 ${isDone ? "bg-green-400" : "bg-border"}`} />
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            ...
                            {stageStartDates[stage.key] && (
                              <span className="ml-1">· planned from {fmt(stageStartDates[stage.key])}</span>
                            )}
                          </p>
                          {log?.enteredAt && (
                            <p className="text-[11px] text-muted-foreground">
                              Entered: {fmt(log.enteredAt)}
                              {log.exitedAt ? ` · Exited: ${fmt(log.exitedAt)}` : ""}
                            </p>
                          )}
                          {/* Photo thumbnails */}
                          {photos.length > 0 && (
                            <div className="flex gap-1.5 mt-2">
                              {photos.slice(0, 2).map((img: string, i: number) => (
                              ...
                              <span className="text-[10px] text-muted-foreground self-end pb-0.5">{photos.length} photo{photos.length !== 1 ? "s" : ""}</span>
                            </div>
                          )}
                        </div>
                        ...
                        {isActive && (
                          ...
                          >
                            {stage.key === "SHAKING_2" ? "Complete & Produce ✓" : "Complete ✓"}
                          </Button>
                        )}
                      </div>
                    );
            ...
            <TabsContent value="output" className="pt-5 space-y-4">
              ...
              <Card className="rounded-sm border-border shadow-none">
                <CardHeader className="pb-2 border-b">
                  <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Stage History</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  {stageLogs.length === 0 ? (
                    <p className="p-8 text-center text-sm text-muted-foreground">No stage transitions recorded yet.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-left whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-border">
                          {stageLogs.map((l: any, i: number) => {
                            ...
                            return (
                              <tr key={l.id ?? i} className="h-[44px] hover:bg-muted/20">
                                <td className="px-4 font-mono font-semibold text-xs">{l.stage.replace(/_/g, " ")}</td>
                                <td className="px-4 font-mono text-xs text-muted-foreground">{fmt(l.enteredAt)}</td>
                                <td className="px-4 font-mono text-xs text-muted-foreground">{fmt(l.exitedAt)}</td>
                                <td className="px-4">
                                  {photos.length >= 2 ? (
                                    <div className="flex gap-1">
                                      {photos.slice(0, 2).map((img: string, pi: number) => (
                                        <button key={pi} type="button" onClick={() => setLightboxSrc(img)}
                                ...
                                <td className="px-4 text-xs text-muted-foreground max-w-[200px] truncate">{l.notes ?? "—"}</td>
                              </tr>
                            );
                          })}
                        </tbody>
              ...
              <Card className="rounded-sm border-border shadow-none">
                <CardHeader className="pb-2 border-b">
                  <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Produced Spawn Stock</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left whitespace-nowrap">
                      <thead className="bg-muted/40 text-muted-foreground text-[10px] uppercase tracking-wider border-b border-border">
                        <tr>
                          ...
                          <th className="px-4 py-2.5 font-medium text-right">Qty (kg)</th>
                      ...
                      <tbody className="divide-y divide-border">
                        {spawnOutputs.map((o: any) => (
                          <tr key={o.id} className="h-[44px] hover:bg-muted/20">
                            ...
                            <td className="px-4 font-mono text-xs text-muted-foreground">{fmt(o.producedAt)}</td>
                            <td className="px-4 font-mono text-right font-semibold">{Number(o.quantityKg).toFixed(1)}</td>
                            <td className="px-4">
                              <div className="flex items-center gap-1.5">
                                {o.status === "stocked" ? (
                                  <><Warehouse className="w-3.5 h-3.5 text-primary" />
                                  <span className="text-xs font-semibold text-primary uppercase tracking-wider">Inventory</span></>
                                ) : (
                                  <><ArrowRight className="w-3.5 h-3.5 text-sky-600" />
                                  <span className="text-xs font-semibold text-sky-700 uppercase tracking-wider">Available for Annur</span></>
                                )}
                              </div>
                            </td>
                          </tr>
                        ...
                        {spawnOutputs.length === 0 && (
                          <tr><td colSpan={4} className="p-8 text-center text-sm text-muted-foreground">
                            No spawn stock produced yet. Complete the final Shaking 2 stage to record output.
                          </td></tr>
                        )}
                      </tbody>
                      {spawnOutputs.length > 0 && (
                        <tfoot className="border-t-2 border-border bg-muted/50">
                          <tr className="h-[44px] font-bold">
                            <td className="px-4 text-xs uppercase tracking-wider" colSpan={2}>Total Produced</td>
      ...
      <Dialog open={advDialog.open} onOpenChange={open => { if (!open) closeAdvDialog(); }}>
        <DialogContent className="rounded-sm border-border max-w-md shadow-xl">
          ...
          <div className="space-y-4 pt-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => setAdvDialog(p => ({ ...p, notes: e.target.value }))}
                className="rounded-sm h-9"
                placeholder="Observations, condition notes…"
              />
            </div>

            {/* SHAKING_2 final stage: destination picker + spawn details */}
            {isFinalStage && (
              <div className="p-3 border border-primary/20 bg-primary/5 rounded-sm space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-primary">Spawn Output</p>

                <div className="space-y-1.5">
                  <Label className="text-xs uppercase tracking-wider text-muted-foreground">Strain Name *</Label>
                  <Input
                    value={advDialog.strainName}
                    onChange={e => setAdvDialog(p => ({ ...p, strainName: e.target.value }))}
                    className="rounded-sm h-9"
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs uppercase tracking-wider text-muted-foreground">Quantity Produced (kg) *</Label>
                  <Input
                    type="number" step="0.1"
                    value={advDialog.spawnQty}
                    onChange={e => setAdvDialog(p => ({ ...p, spawnQty: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button"
                      onClick={() => setAdvDialog(p => ({ ...p, destination: "annur" }))}
                      ...
                      <ArrowRight className={`w-5 h-5 ${advDialog.destination === "annur" ? "text-sky-600" : "text-muted-foreground"}`} />
                      <p className={`text-xs font-semibold ${advDialog.destination === "annur" ? "text-sky-700" : "text-foreground"}`}>
                        Available for Annur
                      </p>
                      <p className="text-[10px] text-muted-foreground">Spawn Mixing source</p>
                    </button>
                    <button type="button"
                      onClick={() => setAdvDialog(p => ({ ...p, destination: "inventory" }))}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-sm border-2 transition-all text-center ${
                        advDialog.destination === "inventory"
                          ? "border-primary bg-primary/5"
                          : "border-border hover:border-primary/50"
                      }`}>
                      <Warehouse className={`w-5 h-5 ${advDialog.destination === "inventory" ? "text-primary" : "text-muted-foreground"}`} />
                      <p className={`text-xs font-semibold ${advDialog.destination === "inventory" ? "text-primary" : "text-foreground"}`}>
                        Stock to Inventory
                      </p>
                      <p className="text-[10px] text-muted-foreground">For sale or later use</p>
                    </button>
                  </div>
                </div>
              </div>
          ...
          <DialogFooter className="pt-2">
            <Button variant="outline" className="rounded-sm" onClick={closeAdvDialog}>
              Cancel
            </Button>
            <Button
