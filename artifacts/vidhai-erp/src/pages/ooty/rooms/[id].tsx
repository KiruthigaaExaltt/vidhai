...
function completedStagesFromPhase(currentPhase: string): Set<string> {
  ...
  if (["CASING_RUN", "DF", "COOKOUT", "COMPLETED"].includes(currentPhase))
    completed.add("SPAWN_RUN");
  if (["DF", "COOKOUT", "COMPLETED"].includes(currentPhase))
    completed.add("CASING_RUN");
...
export default function OotyRoomDetail() {
  ...
  const r = room as any;
  const activeBatch =
    r?.batches?.find((b: any) => b.status === "active") ??
  ...
  const { data: batchDetail } = useGetOotyGrowingBatch(batchId ?? 0, {
    query: {
      enabled: !!batchId,
      queryKey: getGetOotyGrowingBatchQueryKey(batchId ?? 0),
    },
  });
  const b = batchDetail as any;

  // Fetch Coimbatore batches for casing soil selector
  const { data: casingInventory = [] } = useQuery<any[]>({
    queryKey: ["casing-soil-inventory-sources"],
    queryFn: async () => {
  ...
  const refetch = () => {
    ...
    if (batchId)
      queryClient.invalidateQueries({
        queryKey: getGetOotyGrowingBatchQueryKey(batchId),
      });
  };
  ...
  const isCasingRun = completeDialog.stageKey === "CASING_RUN";
  ...
  const selectedCasingSource = casingInventory.find(
    (source: any) =>
  ...
  const harvestReady =
    !isHarvestStage ||
    (Number(completeDialog.harvestWeightKg) > 0 &&
  ...
  const handleCompleteStage = () => {
    ...
    if (isCasingRun) {
      ...
      payload.casingInventorySourceId = Number(
    ...
    if (isCookout) {
      ...
      payload.substrateWeightKg = completeDialog.substrateWeightKg
  ...
  const { completedStageKeys, currentStageKey } = useMemo(() => {
    if (!b)
      return {
        completedStageKeys: new Set<string>(),
        currentStageKey: "SPAWN_RUN",
      };
    ...
    if (b.stageLogs && b.stageLogs.length > 0) {
      const completed = new Set(
        b.stageLogs
          .filter(
            (log: any) => log.exitedAt !== null && log.exitedAt !== undefined,
          )
          .map((log: any) => log.stage),
      );
      const active =
        b.currentStage === "PRONING"
          ? "PINNING_FLUSH1"
          : (b.currentStage ?? "SPAWN_RUN");
  ...
  const isFullyCompleted =
    currentStageKey === "COMPLETED" || b?.status === "completed";
  ...
  const batchSources: any[] =
    b?.batchSources ?? activeBatch?.batchSources ?? [];
  ...
  if (!room)
    return (
      <Shell>
        <div className="p-8 text-sm text-destructive">Room not found.</div>
      </Shell>
    );
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Ooty Location B
              {r.capacity ? ` · ${r.capacity} bags capacity` : ""}
            </p>
          </div>
          {activeBatch && (
            <div className="text-right space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Spawn run started {fmt(activeBatch.spawnRunStartDate)}
              </p>
                ...
                <div className="flex flex-wrap gap-1.5 justify-end mt-1">
                  {batchSources.map((src: any) => (
                    <Badge
                      key={src.id}
                      variant="outline"
                      className="border-primary/30 bg-primary/5 text-primary font-mono text-[11px] rounded-sm gap-1"
                    >
              ...
              <Button
                ...
                onClick={() =>
                  setLocation(`/traceability?batchId=${activeBatch.id}`)
                }
              >
        ...
        {!activeBatch ? (
          <Card className="rounded-sm border-border shadow-none">
            <CardContent className="p-8 text-center text-sm text-muted-foreground">
              Room is idle — no active growing batch. Assign a batch from the
              rooms overview.
            </CardContent>
          </Card>
        ) : (
          <>
                  ...
                  <div className="p-4 border border-primary/20 bg-primary/5 rounded-sm flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Cookout done
                        {b?.cookoutDate ? ` ${fmt(b.cookoutDate)}` : ""}
                        {b?.substrateWeightKg
                          ? ` · spent substrate ${b.substrateWeightKg} kg`
                          : ""}
                        . Room reset to idle.
                      </p>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    <div className="flex gap-2 min-w-max pb-2">
                      {STAGE_SEQ.map((stage, idx) => {
                        ...
                        const isActive =
                          !isFullyCompleted && stage.key === currentStageKey;
                        const isPending = !isCompleted && !isActive;
                        const Icon = stage.icon;

                        // Find stage log entry for completed stages
                        const logEntry = b?.stageLogs?.find(
                          (l: any) => l.stage === stage.key && l.exitedAt,
                        );
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-44 rounded-sm border-2 p-3 transition-all ${
                                isActive
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1.5 mb-1">
                                <Icon
                                  className={`w-3.5 h-3.5 shrink-0 ${isPending ? "text-muted-foreground/50" : isCompleted ? "text-green-700" : "text-primary"}`}
                                />
                                <p
                              ...
                              {isCompleted &&
                                logEntry?.verificationImages?.length > 0 && (
                                  <div className="flex gap-1 mt-2">
                                    {logEntry.verificationImages
                                      .slice(0, 2)
                            ...
                            {idx < STAGE_SEQ.length - 1 && (
                              <div
                                className={`w-4 h-0.5 shrink-0 ${isCompleted ? "bg-green-400" : "bg-border"}`}
                              />
                            )}
                          </div>
                        );
                        ...
                        >
                          <td className="px-4">
                            <span className="text-xs font-semibold uppercase tracking-wider text-foreground">
                              {STAGE_SEQ.find((s) => s.key === log.stage)
                                ?.label ?? log.stage}
                            </span>
                          </td>
            ...
            {/* Harvest Log */}
            {(harvests.length > 0 ||
              ["PINNING_FLUSH1", "FLUSH2", "COOKOUT", "COMPLETED"].includes(
                currentStageKey,
              ) ||
              b?.currentPhase === "DF") && (
              <Card className="rounded-sm border-border shadow-none">
                      ...
                      <tbody className="divide-y divide-border">
                        ...
                        {harvests.length === 0 && (
                          <tr>
                            ...
                            >
                              No harvests logged yet. Flush 1 expected around DF
                              Day 9–11.
                            </td>
                          </tr>
                        )}
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        value={obsForm.temp}
                        onChange={(e) =>
                          setObsForm({ ...obsForm, temp: e.target.value })
                        }
                        className="rounded-sm font-mono h-9 w-[110px]"
                        placeholder="24.5"
                      />
                    </div>
                    <div className="space-y-1 flex-1 min-w-[200px]">
                      ...
                      <Input
                        value={obsForm.note}
                        onChange={(e) =>
                          setObsForm({ ...obsForm, note: e.target.value })
                        }
                        className="rounded-sm h-9"
                        placeholder="Mycelium spread, humidity, CO₂..."
                      />
                    </div>
                    <Button
                      type="submit"
                      ...
                      {observations.map((o: any) => (
                        <tr key={o.id} className="h-[36px] hover:bg-muted/20">
                          ...
                          <td className="px-4 font-mono text-right">
                            ...
                            o.temperatureCelsius !== undefined
                              ? Number(o.temperatureCelsius).toFixed(1)
                              : "—"}
                          </td>
      ...
      >
        <DialogContent className="rounded-sm border-border max-w-md shadow-xl">
          ...
          <div className="space-y-4 pt-1 max-h-[60vh] overflow-y-auto pr-1">
            ...
            {/* Casing Run consumes one exact produced batch or purchased lot. */}
            {isCasingRun && (
              <div className="p-3 bg-muted/30 border border-border rounded-sm space-y-3">
                ...
                >
                  <div className="flex items-center gap-2 bg-white px-3 py-2 border rounded-sm flex-1">
                    ...
                    <Label htmlFor="casing-produced">Produced</Label>
                  </div>
                  <div className="flex items-center gap-2 bg-white px-3 py-2 border rounded-sm flex-1">
                ...
                >
                  <SelectTrigger className="rounded-sm h-9 bg-white">
                    <SelectValue
                      placeholder={
                        completeDialog.casingSourceType === "produced"
                          ? "Select produced batch"
                          : "Select purchased lot"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {casingInventory
                      .filter(
                        (source: any) =>
                          source.sourceType ===
                            completeDialog.casingSourceType &&
                          Number(source.availableQuantityKg) > 0,
                      )
                      .map((source: any) => (
                ...
                <div className="space-y-1.5">
                  <Label className="text-[11px] uppercase tracking-wider text-muted-foreground">
                    Casing Soil Quantity Used (kg) *
                  </Label>
                  <Input
                    type="number"
                    min="0.0001"
                    step="0.0001"
                    value={completeDialog.casingQuantityKg}
                    onChange={(event) =>
                      setCompleteDialog((previous) => ({
                        ...previous,
                        casingQuantityKg: event.target.value,
                      }))
                    }
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <span className="font-mono font-semibold">
                        {Number(
                          selectedCasingSource.availableQuantityKg,
                        ).toFixed(2)}{" "}
                        kg
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-mono font-semibold">
                        {Math.max(
                          0,
                          Number(selectedCasingSource.availableQuantityKg) -
                            (Number.isFinite(casingQuantity)
                              ? casingQuantity
                              : 0),
                        ).toFixed(2)}{" "}
            ...
            {isHarvestStage && (
              <div className="p-3 bg-emerald-50/50 border border-emerald-200 rounded-sm space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-emerald-800">
                  {completeDialog.stageKey === "PINNING_FLUSH1"
                    ? "Flush 1 Harvest Data"
                    : "Flush 2 Harvest Data"}
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCompleteDialog((p) => ({
                          ...p,
                          harvestWeightKg: e.target.value,
                        }))
                      }
                      className="rounded-sm h-8 font-mono text-sm"
                    />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCompleteDialog((p) => ({
                          ...p,
                          harvestQualityNote: e.target.value,
                        }))
                      }
                ...
                {!completeDialog.harvestWeightKg && (
                  <p className="text-xs text-amber-600 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> Harvest weight
                    required
                  </p>
                )}
              </div>
            ...
            {isCookout && (
              <div className="p-3 bg-red-50/50 border border-red-200 rounded-sm space-y-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Records spent substrate weight, completes the batch, and
                  resets the room to idle.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCompleteDialog((p) => ({
                          ...p,
                          substrateWeightKg: e.target.value,
                        }))
                      }
                      className="rounded-sm h-8 font-mono text-sm"
                    />
                ...
                {!completeDialog.substrateWeightKg && (
                  <p className="text-xs text-amber-600 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> Substrate weight
                    required
                  </p>
                )}
                {(completeDialog.manureKg === "" ||
          ...
          <DialogFooter className="pt-2">
            ...
            >
              {advanceStageMutation.isPending
                ? "Saving…"
                : !casingReady
                  ? "Select casing stock and quantity"
                  : !harvestReady
                    ? "Enter harvest weight and mushroom count"
                    : !cookoutReady
                      ? "Enter substrate and Manure weight"
                      : "Mark Stage Complete ✓"}
            </Button>
          </DialogFooter>
        </DialogContent>
