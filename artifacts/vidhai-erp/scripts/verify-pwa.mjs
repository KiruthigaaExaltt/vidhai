...
for(const file of ["index.html","sw.js","manifest.webmanifest","pwa-maharishi-v4-192x192.png","pwa-maharishi-v4-512x512.png","pwa-maharishi-v4-maskable-512x512.png","apple-touch-icon-maharishi-v4.png","favicon-maharishi-v4.png"])assert.ok(existsSync(resolve(dist,file)),`${file} missing`);
const manifest=JSON.parse(readFileSync(resolve(dist,"manifest.webmanifest"),"utf8"));
assert.equal(manifest.id,"/vidhai-erp-maharishi-v4");
assert.equal(manifest.name,"Vidhai ERP Production Control Center");
assert.equal(manifest.display,"standalone");
assert.equal(manifest.start_url,"/?pwa=maharishi-v4");
assert.equal(manifest.theme_color,"#20BFAF");
assert.equal(manifest.icons.length,3);
for(const icon of manifest.icons)assert.ok(icon.src.includes("maharishi-v4"),`unversioned icon URL: ${icon.src}`);
