...
export async function effectivePermissions(user: AuthUser): Promise<string[]> {
  ...
  const roles = await db.select().from(rolesTable).where(eq(rolesTable.organizationId, user.organizationId ?? 1));
  const role = roles.find((item: any) => item.isActive !== false && item.name === user.role);
  ...
  for (const override of json<any[]>(user.permissionOverrides, [])) {
    ...
    override.allowed ? base.add(override.permissionKey) : base.delete(override.permissionKey);
  }
  return [...base];
}
...
export function requirePermission(permission: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions.includes("*") && !permissions.includes(permission)) return res.status(403).json({ error: `Missing permission: ${permission}` });
