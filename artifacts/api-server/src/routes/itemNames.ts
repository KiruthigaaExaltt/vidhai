...
router.get("/", async (req, res) => {
  ...
  const items = await db
    ...
    .innerJoin(
      inventoryCategoriesTable,
      eq(itemNamesTable.categoryId, inventoryCategoriesTable.id),
    )
  ...
  if (String(req.query.coreOnly || "").toLowerCase() === "true")
    data = data.filter((row) => isCoreProductMasterItem(row.name));
...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ error: "An active item with this name already exists" });
      return;
    }

    const [newItem] = await db
      .insert(itemNamesTable)
      .values({
        name: trimmedName,
  ...
  try {
    ...
    if (name) {
      ...
      const existing = await db
        .select()
        .from(itemNamesTable)
        .where(and(eq(itemNamesTable.isActive, true)));
      if (
        existing.some(
          (i) =>
            i.id !== id && i.name.toLowerCase() === trimmedName.toLowerCase(),
        )
      ) {
        res
          .status(400)
    ...
    if (categoryId && Number(categoryId) !== item.categoryId) {
      ...
      const skusUsingThisItem = await db
        .select()
        .from(materialsTable)
