...
import {
  dispatchVaultStockDirect,
  dispatchVaultReservation,
  listVaultSalesStock,
} from "../lib/vaultSales";
...
router.get("/vault-sales-stock", requireAuth, async (_req, res) => {
  return res.json(await listVaultSalesStock());
});
...
const quotationItemNumericFields = [
  "quantity",
  "rate",
  "discountPercent",
  "cgstPercent",
...
const vaultLineFields = (item: any) => ({
  vaultType: item.vaultType || null,
  vaultSourceType: item.vaultSourceType || null,
  vaultStockId: item.vaultStockId ? Number(item.vaultStockId) : null,
  vaultReference: item.vaultReference || null,
  vaultReservationId: item.vaultReservationId
    ? Number(item.vaultReservationId)
    : null,
  sourceLineId: item.sourceLineId ? Number(item.sourceLineId) : null,
});

async function salesLinesWithReservations(
  items: any[],
  ...
  const reservations = await db
    .select()
    .from(vaultSalesReservationsTable)
    .where(
      and(
        eq(vaultSalesReservationsTable.sourceDocumentType, sourceDocumentType),
        eq(vaultSalesReservationsTable.sourceDocumentId, sourceDocumentId),
      ),
    );
  return items.map((item) => {
    const serialized = serializeQuotationItem(item);
    const reservation = reservations.find(
      (row) => Number(row.sourceLineId) === Number(item.id),
    );
    return {
      ...serialized,
      sourceLineId: item.id,
      vaultReservationId: reservation?.id ?? item.vaultReservationId ?? null,
      deliveredQuantity: reservation
...
function serializeProforma<T extends Record<string, any>>(row: T) {
  const result: Record<string, any> = { ...row };
  for (const field of proformaNumericFields)
    result[field] =
      Number(
...
async function saveProformaItems(piId: number, items: any[]) {
  await db
    ...
    .where(eq(proformaInvoiceItemsTable.piId, piId));
  for (const item of items || []) {
    await db.insert(proformaInvoiceItemsTable).values({
      ...
      cgstPercent: String(item.cgstPercent ?? 0),
      sgstPercent: String(item.sgstPercent ?? 0),
      igstPercent: String(item.igstPercent ?? 0),
...
function proformaData(payload: any) {
  return {
    ...
    customerWhatsappNumber:
      payload.customerWhatsappNumber || payload.customerMobile || "",
    customerCompany: payload.customerCompany || "",
    customerAddress: payload.customerAddress || "",
    shippingAddress: payload.shippingAddress ?? payload.customerAddress ?? "",
    customerGstin: payload.customerGstin || "",
    customerCountryCode: payload.customerCountryCode || "91",
    placeOfSupply: payload.placeOfSupply || "",
    piDate: payload.proformaDate || payload.piDate || new Date().toISOString(),
    validUntil: payload.validUntil || null,
    subtotal: String(payload.subtotal ?? 0),
    taxableAmount: String(payload.taxableAmount ?? payload.subtotal ?? 0),
    ...
    terms: payload.termsAndConditions ?? payload.terms ?? "",
...
function challanData(payload: any) {
  return {
    quotationIds: Array.isArray(payload.quotationIds)
      ? payload.quotationIds.map(Number).filter(Boolean)
      : payload.quoteId
        ? [Number(payload.quoteId)]
        : [],
    piIds: Array.isArray(payload.piIds)
      ? payload.piIds.map(Number).filter(Boolean)
      : payload.piId
        ? [Number(payload.piId)]
        : [],
    clientId: Number(payload.clientId),
    ...
    customerGstin: payload.customerGstin || "",
    placeOfSupply: payload.placeOfSupply || "",
    dcDate: payload.challanDate || payload.dcDate || new Date().toISOString(),
    deliveryDate: payload.deliveryDate || payload.validUntil || null,
    subtotal: String(payload.subtotal ?? 0),
    cgstTotal: String(payload.cgstTotal ?? 0),
    sgstTotal: String(payload.sgstTotal ?? 0),
    igstTotal: String(payload.igstTotal ?? 0),
...
async function saveChallanItems(dcId: number, items: any[]) {
  await db
    ...
    .where(eq(deliveryChallanItemsTable.dcId, dcId));
  for (const item of items || [])
...
function invoiceData(payload: any) {
  const ids = (value: any, fallback?: any) =>
    Array.isArray(value)
      ? [...new Set(value.map(Number).filter((id: number) => id > 0))]
      : fallback
        ? [Number(fallback)]
        : [];
  return {
    quotationIds: ids(
      payload.quotationIds ?? payload.quoteIds,
      payload.quoteId,
    ),
    piIds: ids(payload.piIds, payload.piId),
    dcIds: ids(payload.dcIds, payload.dcId),
    clientId: Number(payload.clientId),
    clientName: payload.clientName || "",
    customerMobile: payload.customerMobile || "",
    customerWhatsappNumber:
    ...
    dueDate: payload.dueDate || payload.validUntil || null,
    subtotal: String(payload.subtotal ?? 0),
    taxableAmount: String(payload.taxableAmount ?? payload.subtotal ?? 0),
    cgstTotal: String(payload.cgstTotal ?? 0),
    sgstTotal: String(payload.sgstTotal ?? 0),
    ...
    grandTotal: String(payload.grandTotal ?? 0),
    discountAmount: String(payload.discountAmount ?? 0),
    transportCharges: String(payload.transportCharges ?? 0),
    amountPaid: String(payload.amountPaid ?? 0),
    balanceDue: String(payload.balanceDue ?? payload.grandTotal ?? 0),
...
async function invoiceWithItems(doc: any) {
  const items = await db
    ...
    .where(eq(salesInvoiceItemsTable.invoiceId, doc.id));
  const activeReturns = (await db.select().from(salesReturnsTable)).filter(
    (row) =>
      Number(row.invoiceId) === Number(doc.id) &&
  ...
  return {
    ...
    items: items.map((item) => {
      ...
      const alreadyReturnedQty =
        returnedByInvoiceItem.get(Number(item.id)) || 0;
      return {
        ...serialized,
        alreadyReturnedQty,
        returnableQty: Math.max(
          0,
          Number(serialized.quantity || 0) - alreadyReturnedQty,
        ),
      };
    }),
  };
...
async function publishAccountsReceivableEntryNotification(
  req: any,
  entry: any,
) {
...
router.post("/", requireAuth, async (req, res) => {
  ...
  const [row] = await db
    ...
    .values({
      orderCode: code,
      productType,
      saleType: saleType ?? "external",
      transactionDate,
      qtyKg: String(qtyKg),
      unit: unit ?? "kg",
      buyerName: buyerName ?? null,
      destinationLocationId: destinationLocationId ?? null,
      fromBatchId: fromBatchId ?? null,
      fromBatchCode: fromBatchCode ?? null,
    ...
    .returning();
  await publishNotification({
    organizationId: Number((req as any).authUser?.organizationId ?? 1),
    actorId: Number(userId),
    permissionKey: "sales.quotations.notification",
    eventType: "SALES_ORDER_CREATED",
    eventKey: `sales-order:${row.id}:created`,
    sourceModule: "sales",
    targetModule: "sales",
    submodule: "sales_orders",
    title: "Sales order created",
    message: `${row.orderCode || `Sales order ${row.id}`} was created.`,
    sourceEntityType: "sales_order",
    sourceEntityId: row.id,
    sourceReference: row.orderCode,
    navigationUrl: "/sales",
  });
  res.locals.notificationHandled = true;
...
function throwIfQuotationLocked(status: string) {
  if (isQuotationLocked(status)) {
    const error = new Error(
      "This quotation is approved or rejected and can no longer be edited or re-sent.",
    );
...
async function nextSeriesVersionNumber(
  rootQuoteNumber: string,
  series: "Draft" | "Sent",
): Promise<number> {
  const rows = await db
  ...
  const seriesRows = rows.filter(
    (r) => resolveVersionSeries(r.status) === series,
  );
  return (
    seriesRows.reduce((max, r) => Math.max(max, r.versionNumber || 1), 0) + 1
  );
}
...
async function copyQuotationItems(
  ...
  const items = await db
    ...
    .where(eq(quotationItemsTable.quoteId, sourceQuotationId));
  for (const item of items) {
...
async function createQuotationVersion(
  ...
  const [source] = await db
    .select()
    .from(quotationsTable)
    .where(eq(quotationsTable.id, sourceQuotationId))
    .limit(1);
  ...
  const preservedStatus =
    rawStatus === "Approved"
      ? series === "Sent"
        ...
        : "Draft"
      : rawStatus;
  const rootQuoteNumber =
    source.rootQuoteNumber ||
...
router.get("/quotations", requireAuth, async (req, res) => {
  ...
  if (!forMapping) {
    ...
    const rows = await db
      .select()
      .from(quotationsTable)
      .where(eq(quotationsTable.isLatestVersion, true))
...
router.post("/quotations", requireAuth, async (req, res) => {
  ...
  await publishSalesCreatedNotification(req, res, {
    permissionKey: "sales.quotations.notification",
    viewPermissionKey: "sales.quotations.view",
    eventType: "SALES_QUOTATIONS_DRAFT",
    title: "Quotation created",
    message: `${doc.quoteNumber || `Quotation ${doc.id}`} was created for ${doc.clientName || "the customer"}.`,
    entityType: "quotation",
    entityId: Number(doc.id),
    reference: doc.quoteNumber || String(doc.id),
  });
  return res.status(201).json({
    ...serializeQuotation(doc),
...
router.get("/quotations/:id", requireAuth, async (req, res) => {
  ...
  const items = await db
    .select()
    .from(quotationItemsTable)
    .where(eq(quotationItemsTable.quoteId, id));
  return res.json({
    ...serializeQuotation(doc),
    items: await salesLinesWithReservations(items, "Quotation", id),
  });
});
...
router.patch("/quotations/:id", requireAuth, async (req, res) => {
  ...
  try {
    const { targetId, forked, quotation } = await resolveQuotationPatchTarget(
      existing,
      userId,
      {
    ...
    for (const f of fields) {
      if (req.body[f] !== undefined) {
        updates[f] =
          typeof req.body[f] === "number" || typeof req.body[f] === "boolean"
            ? req.body[f]
            : String(req.body[f]);
      }
    }
    ...
    if (Array.isArray(items) && items.length > 0) {
      await db
        ...
        .where(eq(quotationItemsTable.quoteId, targetId));
      for (const item of items) {
        await db.insert(quotationItemsTable).values({
          quoteId: targetId,
...
router.delete("/quotations/:id", requireAuth, async (req, res) => {
  ...
  const siblings = await db
    .select()
    .from(quotationsTable)
    .where(eq(quotationsTable.rootQuoteNumber, document.rootQuoteNumber));
  for (const sibling of siblings) {
    await db
      .delete(quotationItemsTable)
      .where(eq(quotationItemsTable.quoteId, sibling.id));
    await db
      .delete(quotationCommunicationsTable)
      .where(eq(quotationCommunicationsTable.quotationId, sibling.id));
...
router.get("/quotations/:id/communications", requireAuth, async (req, res) => {
  ...
  const rows = await db
    .select()
    .from(quotationCommunicationsTable)
    .where(
      eq(quotationCommunicationsTable.rootQuoteNumber, doc.rootQuoteNumber),
    )
...
router.get("/quotations/:id/contact-details", requireAuth, async (req, res) => {
  ...
  const contacts = await db
    .select()
    .from(contactsTable)
    .where(eq(contactsTable.id, doc.clientId))
    .limit(1);
...
router.post("/quotations/:id/send", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (mobile.length !== 10)
      return res
        .status(400)
        .json({ error: "Valid 10-digit mobile number is required" });

    const currentSeries = resolveVersionSeries(existing.status);
    const rootQuoteNumber =
      existing.rootQuoteNumber ||
    ...
    const extraFields = {
      ...
      customerCountryCode: String(req.body.countryCode || "91").replace(
        /\D/g,
        "",
      ),
      ...
      customerWhatsappNumber:
        req.body.customerWhatsappNumber ?? existing.customerWhatsappNumber,
      ...
      discountAmount: String(
        req.body.discountAmount ?? existing.discountAmount,
      ),
      ...
      billedByCompanyName:
        req.body.billedByCompanyName ?? existing.billedByCompanyName,
      ...
      billedByContactNumber:
        req.body.billedByContactNumber ?? existing.billedByContactNumber,
    };
    ...
    if (Array.isArray(items) && items.length > 0) {
      await db
        .delete(quotationItemsTable)
        .where(eq(quotationItemsTable.quoteId, newQuote.id));
      for (const item of items) {
        await db.insert(quotationItemsTable).values({
          quoteId: newQuote.id,
          quotationId: newQuote.id,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      !["confirm", "reject", "call", "message", "negotiation"].includes(action)
    ) {
    ...
    try {
      ...
      if (action === "confirm") {
        if (existing.status !== "Sent") {
          return res.status(400).json({
            error: "Customer confirmation is only allowed for sent quotations",
          });
        }
        nextStatus = "Approved";
        ...
        if (existing.status !== "Sent") {
          return res.status(400).json({
            error: "Customer rejection is only allowed for sent quotations",
          });
        }
        nextStatus = "Rejected";
      ...
      if (action === "confirm") {
        const [alreadyCreated] = await db
          ...
          .where(eq(proformaInvoicesTable.autoCreatedFromQuotationId, id))
          .limit(1);
        if (alreadyCreated) {
          const piItems = await db
            .select()
            .from(proformaInvoiceItemsTable)
            .where(eq(proformaInvoiceItemsTable.piId, alreadyCreated.id));
          autoCreatedProforma = {
            ...serializeProforma(alreadyCreated),
            items: piItems.map(serializeProformaItem),
          };
        } else {
          const existingPiCount = (
            await db.select().from(proformaInvoicesTable)
          ).length;
...
router.get("/approved-quotations", requireAuth, async (req, res) => {
  ...
  const startedSources = new Set(
    workOrders.map(
      (row) => `${row.sourceDocumentType}:${row.sourceDocumentId}`,
    ),
  );
  ...
  for (const row of inventory) {
    ...
    stock.set(
      ...
      (stock.get(Number(row.materialId)) || 0) + quantity,
    );
  }
  for (const active of workOrders.filter(
    (row: any) =>
      !["Completed", "Cancelled", "Rejected"].includes(String(row.status)),
  )) {
    for (const reserved of Array.isArray(active.materialRequirements)
      ? active.materialRequirements
      : []) {
      const materialId = Number(reserved.materialId);
      const quantity = Number(reserved.requiredQuantity || 0) || 0;
      if (materialId)
        stock.set(materialId, (stock.get(materialId) || 0) - quantity);
    }
  }
  const result: any[] = [];
    ...
    const insufficientItems = serializedItems
      .filter(
        (item: any) =>
          ...
          !item.serviceId &&
          item.itemId &&
          (stock.get(Number(item.itemId)) || 0) < Number(item.quantity || 0),
      )
      .map((item: any) => ({
        itemId: item.itemId,
        description: item.description || item.productName,
        required: Number(item.quantity || 0),
        available: stock.get(Number(item.itemId)) || 0,
      }));
    result.push({
      ...serializeProforma(doc),
      source,
      documentNumber:
        source === "Quotation"
          ? doc.rootQuoteNumber || doc.quotationNumber || doc.quoteNumber
          : doc.rootPiNumber || doc.piNumber,
      customerApprovedAt: doc.customerResponseAt,
      versionNumber: Number(doc.versionNumber || 1),
      versionLabel:
        doc.versionLabel ||
        `${doc.versionSeries || "Sent"} V${Number(doc.versionNumber || 1)}`,
  ...
  for (const doc of quotes.filter(
    (row) =>
      row.isLatestVersion !== false &&
      row.status === "Approved" &&
      row.customerResponseAt,
  ))
    await append(doc, "Quotation");
  for (const doc of proformas.filter(
    (row) =>
      row.isLatestVersion !== false &&
      row.status === "Approved" &&
      row.customerResponseAt &&
      !row.autoCreatedFromQuotationId,
  ))
    await append(doc, "Proforma Invoice");
  result.sort(
    (left, right) =>
...
router.post("/approved-quotations/reject", requireAuth, async (req, res) => {
  ...
  if (!["Quotation", "Proforma Invoice"].includes(source) || !documentId)
    return res
      .status(400)
      .json({ error: "A valid approved document is required" });
  if (!rejectionReason)
    return res.status(400).json({ error: "Rejection reason is required" });
  const table =
    source === "Quotation" ? quotationsTable : proformaInvoicesTable;
  const [existing] = await db
    .select()
    .from(table)
  ...
  if (!existing || existing.status !== "Approved")
    return res
      .status(409)
      .json({ error: "Only an approved document can be rejected" });
  const [workOrder] = await db
    .select()
    .from(salesWorkOrdersTable)
    .where(
      and(
        eq(salesWorkOrdersTable.sourceDocumentType, source),
        eq(salesWorkOrdersTable.sourceDocumentId, documentId),
      ),
    )
    .limit(1);
  if (workOrder)
    return res.status(409).json({
      error:
        "This document already has a Work Order and cannot be rejected from the queue",
    });
  const [updated] = await db
    .update(table)
    .set({
...
router.get("/proforma-invoices", requireAuth, async (req, res) => {
  ...
  return listResponse(
    req,
    res,
    rows.filter((row) => row.isLatestVersion !== false).map(serializeProforma),
  );
});
...
router.post("/proforma-invoices", requireAuth, async (req, res) => {
  try {
    ...
    return res.status(201).json({
      ...serializeProforma(doc),
      items: items.map(serializeProformaItem),
    });
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
...
router.get("/proforma-invoices/:id", requireAuth, async (req, res) => {
  ...
  return res.json({
    ...serializeProforma(doc),
    items: await salesLinesWithReservations(items, "Proforma Invoice", id),
  });
});
...
router.get("/proforma-invoices/:id/versions", requireAuth, async (req, res) => {
  ...
  let rows = await db
    ...
    .from(proformaInvoicesTable)
    .where(
      eq(proformaInvoicesTable.rootPiNumber, doc.rootPiNumber || doc.piNumber),
    )
    .orderBy(desc(proformaInvoicesTable.createdAt));
...
router.patch("/proforma-invoices/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (!source)
      return res.status(404).json({ error: "Proforma invoice not found" });
    if (["Approved", "Rejected"].includes(source.status))
      return res
        .status(409)
        .json({ error: "Approved or rejected Proforma invoices are locked" });
    const root = source.rootPiNumber || source.piNumber;
    let siblings = await db
      .select()
      .from(proformaInvoicesTable)
      .where(eq(proformaInvoicesTable.rootPiNumber, root));
    if (!siblings.length) siblings = [source];
    const next =
      Math.max(
        0,
        ...siblings
          .filter((row) => row.versionSeries === "Draft")
          .map((row) => Number(row.versionNumber || 0)),
      ) + 1;
    await db
    ...
    await db
      .update(proformaInvoicesTable)
      .set({ isLatestVersion: false })
      .where(eq(proformaInvoicesTable.id, source.id));
    const [doc] = await db
      .insert(proformaInvoicesTable)
      .values({
        piNumber: root,
        rootPiNumber: root,
        ...proformaData({ ...source, ...payload }),
        status: "Draft",
        versionSeries: "Draft",
        versionNumber: next,
        versionLabel: `Draft V${next}`,
        parentPiId: source.id,
        isLatestVersion: true,
        isLocked: false,
  ...
  try {
    ...
    const next =
      Math.max(
        0,
        ...siblings
          .filter((row) => row.versionSeries === "Sent")
          .map((row) => Number(row.versionNumber || 0)),
      ) + 1;
    await db
      .update(proformaInvoicesTable)
...
router.post(
  "/proforma-invoices/:id/customer-response",
  requireAuth,
  async (req, res) => {
    const id = Number(req.params.id);
    ...
    if (!doc)
      return res.status(404).json({ error: "Proforma invoice not found" });
    if (doc.autoCreatedFromQuotationId)
      return res.status(409).json({
        error: "Customer approval was already recorded on the source quotation",
      });
    if (doc.status !== "Sent")
      return res
        .status(400)
        .json({ error: "Customer response is only available after sending" });
    if (!["confirm", "reject"].includes(action))
      return res.status(400).json({ error: "Invalid customer response" });
    ...
    const [updated] = await db
      ...
      .set({
        ...
        confirmedByUserId: (req.session as any).userId,
...
router.delete("/proforma-invoices/:id", requireAuth, async (req, res) => {
  ...
  for (const sibling of siblings) {
    await db
      .delete(proformaInvoiceItemsTable)
      .where(eq(proformaInvoiceItemsTable.piId, sibling.id));
    await db
      .delete(proformaInvoicesTable)
      .where(eq(proformaInvoicesTable.id, sibling.id));
  }
  return res.status(204).send();
});
...
router.get("/challans", requireAuth, async (req, res) => {
  const rows = await db
    ...
    .orderBy(desc(deliveryChallansTable.createdAt));
...
router.post("/challans", requireAuth, async (req, res) => {
  try {
    if (!req.body?.clientId)
      return res.status(400).json({ error: "Client is required" });
    const dcNumber = dcCode(
      (await db.select().from(deliveryChallansTable)).length + 1,
    );
    const [doc] = await db
      .insert(deliveryChallansTable)
      .values({
        dcNumber,
        ...challanData(req.body),
        status: "Draft",
        stockDeducted: false,
      })
      .returning();
    ...
    const items = await db
      ...
      .where(eq(deliveryChallanItemsTable.dcId, doc.id));
    await publishSalesCreatedNotification(req, res, {
      permissionKey: "sales.delivery_challans.notification",
      viewPermissionKey: "sales.delivery_challans.view",
      eventType: "SALES_CHALLANS_DRAFT",
      ...
      entityId: Number(doc.id),
      reference: doc.dcNumber || String(doc.id),
    });
    return res.status(201).json({
      ...serializeProforma(doc),
      items: items.map(serializeQuotationItem),
    });
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
...
router.get("/challans/:id", requireAuth, async (req, res) => {
  ...
  const items = await db
    .select()
    .from(deliveryChallanItemsTable)
    .where(eq(deliveryChallanItemsTable.dcId, id));
  return res.json({
    ...serializeProforma(doc),
    items: items.map(serializeQuotationItem),
  });
...
router.patch("/challans/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (existing.status === "Dispatched")
      return res.status(409).json({
        error:
          "Dispatched delivery challans are locked. Create a Sales Return to reverse stock.",
      });
    const [doc] = await db
      .update(deliveryChallansTable)
      .set({ ...challanData({ ...existing, ...req.body }), status: "Draft" })
      .where(eq(deliveryChallansTable.id, id))
      .returning();
...
router.post("/challans/:id/send", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (doc.stockDeducted || doc.status === "Dispatched") {
      ...
      return res.json({
        ...serializeProforma(doc),
        items: existingItems.map(serializeQuotationItem),
      });
    }
    if (Array.isArray(req.body.items)) {
      [doc] = await db
        .update(deliveryChallansTable)
        .set(challanData({ ...doc, ...req.body }))
        .where(eq(deliveryChallansTable.id, id))
        .returning();
      await saveChallanItems(id, req.body.items);
    }
    ...
    await db.transaction(async (tx) => {
      ...
      if (
        !lockedDoc ||
        lockedDoc.stockDeducted ||
        lockedDoc.status === "Dispatched"
      )
        return;
      ...
      for (const item of items) {
        ...
        if (!(quantity > 0))
          throw new Error(
            `Dispatch quantity must be greater than zero for ${item.description || item.productName}`,
          );

        if (item.vaultType) {
          if (item.vaultReservationId) {
            const reservation = await dispatchVaultReservation(
              tx,
              Number(item.vaultReservationId),
              quantity,
              lockedDoc,
            ...
            if (
              reservation.vaultType !== item.vaultType ||
              Number(reservation.vaultStockId) !== Number(item.vaultStockId)
            )
              throw new Error(
                "DC Vault batch does not match the reserved Sales batch",
              );
          } else {
            await dispatchVaultStockDirect(
              tx,
              item,
              quantity,
              lockedDoc,
          ...
          await tx
            .update(deliveryChallanItemsTable)
            .set({ dispatchedQty: String(quantity) })
            .where(eq(deliveryChallanItemsTable.id, item.id));
          continue;
        }

        const materialId = Number(item.itemId || item.productId);
        const warehouseId = Number(item.warehouseId);
        if (!materialId || !warehouseId)
          throw new Error(
            `Warehouse is required for ${item.description || item.productName}`,
          );
        ...
        if (!inventory || available < quantity)
          throw new Error(
            `Insufficient stock for ${item.description || item.productName} in ${item.warehouseName || `warehouse #${warehouseId}`}. Available ${available}, requested ${quantity}.`,
          );
        await tx
          .update(inventoryTable)
          .set({
            quantityOnHand: String(available - quantity),
        ...
        await tx.insert(inventoryMovementsTable).values({
          ...
          notes: `Delivery Challan ${lockedDoc.dcNumber}`,
          createdByUserId: userId,
        });
        await tx
          .update(deliveryChallanItemsTable)
...
router.delete("/challans/:id", requireAuth, async (req, res) => {
  ...
  if (doc.status === "Dispatched")
    return res.status(409).json({
      error:
        "Dispatched delivery challans cannot be deleted. Create a Sales Return instead.",
    });
  await db
    .delete(deliveryChallanItemsTable)
    .where(eq(deliveryChallanItemsTable.dcId, id));
...
router.get("/invoices", requireAuth, async (req, res) => {
  ...
  for (const invoice of rows.filter(
    (row) => row.isLatestVersion && ["Approved", "Paid"].includes(row.status),
  ))
  ...
  const latest = rows.filter(
    (row) => row.isLatestVersion || !row.rootInvoiceNumber,
  );
...
router.post("/invoices", requireAuth, async (req, res) => {
  try {
    ...
    const families = [
      req.body.quotationIds || req.body.quoteIds,
      req.body.piIds,
      req.body.dcIds,
    ...
    if (families.length > 1)
      return res.status(400).json({
        error:
          "Select only one mapping type: Quotations, Proforma Invoices, or Delivery Challans",
      });
    const invoiceNumber = invoiceCode(
      (await db.select().from(salesInvoicesTable)).length + 1,
    );
    ...
    const [doc] = await db
      ...
      .values({
        ...
        ...invoicePayload,
        amountPaid: "0",
        balanceDue: invoicePayload.grandTotal,
...
router.get("/invoices/:id", requireAuth, async (req, res) => {
  const [doc] = await db
    .select()
    .from(salesInvoicesTable)
    .where(eq(salesInvoicesTable.id, Number(req.params.id)))
    .limit(1);
...
router.get("/invoices/:id/versions", requireAuth, async (req, res) => {
  ...
  let rows = await db
    ...
    .where(
      eq(
        salesInvoicesTable.rootInvoiceNumber,
        doc.rootInvoiceNumber || doc.invoiceNumber,
      ),
    );
  if (!rows.length) rows = [doc];
  return res.json({
...
async function createInvoiceRevision(
  source: any,
  payload: any,
  series: "Draft" | "Sent",
) {
  ...
  let siblings = await db
    .select()
    .from(salesInvoicesTable)
    .where(eq(salesInvoicesTable.rootInvoiceNumber, root));
  if (!siblings.length) siblings = [source];
  const versionNumber =
    Math.max(
      0,
      ...siblings
        .filter((row) => row.versionSeries === series)
        .map((row) => Number(row.versionNumber || 0)),
    ) + 1;
  await db
  ...
  await db
    .update(salesInvoicesTable)
    .set({ isLatestVersion: false })
    .where(eq(salesInvoicesTable.id, source.id));
  const [doc] = await db
    .insert(salesInvoicesTable)
    .values({
      invoiceNumber: root,
      rootInvoiceNumber: root,
      ...invoiceData({ ...source, ...payload }),
      status: series,
      sentAt: series === "Sent" ? new Date() : null,
      versionSeries: series,
      versionNumber,
      versionLabel: `${series} V${versionNumber}`,
      parentInvoiceId: source.id,
      isLatestVersion: true,
      isLocked: false,
      changeRemarks: payload.changeRemarks || "",
    })
    .returning();
  const sourceItems = await db
    .select()
    .from(salesInvoiceItemsTable)
    .where(eq(salesInvoiceItemsTable.invoiceId, source.id));
  await saveInvoiceItems(
    Number(doc.id),
    Array.isArray(payload.items) ? payload.items : sourceItems,
  );
...
router.patch("/invoices/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (
      source.isLocked ||
      ["Approved", "Rejected", "Paid", "Cancelled"].includes(source.status)
    )
      return res
        .status(409)
        .json({ error: "Confirmed or closed invoices cannot be edited" });
    return res.json(
      await createInvoiceRevision(source, req.body || {}, "Draft"),
    );
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
...
router.post("/invoices/:id/send", requireAuth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(409)
        .json({ error: "Confirmed or closed invoices cannot be re-sent" });
    return res.json(
      await createInvoiceRevision(source, req.body || {}, "Sent"),
    );
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (doc.status !== "Sent")
      return res
        .status(400)
        .json({ error: "Customer response is only allowed for sent invoices" });
    if (!["confirm", "reject"].includes(action))
      return res.status(400).json({ error: "Invalid customer response" });
    const status = action === "confirm" ? "Approved" : "Rejected";
    const [updated] = await db
    ...
    if (status === "Approved") {
      try {
        const context = await accountingContext(req);
        const accounting = await triggerInvoiceApproved(
          id,
        ...
        await publishNotification({
          ...
          metadata: {
            status: "Approved",
            accountsReceivableId: accounting?.ar?.id,
          },
        });
        res.locals.notificationHandled = true;
      } catch (error: any) {
        ...
        return res.status(500).json({
          error: `Invoice approval accounting failed: ${error.message}`,
        });
      }
    }
...
router.delete("/invoices/:id", requireAuth, async (req, res) => {
  ...
  if (
    doc.isLocked ||
    ["Approved", "Rejected", "Paid", "Cancelled"].includes(doc.status)
  )
    return res
      .status(409)
      .json({ error: "Confirmed or closed invoices cannot be deleted" });
  let siblings = await db
    .select()
    .from(salesInvoicesTable)
    .where(
  ...
  for (const sibling of siblings) {
    await db
      .delete(salesInvoiceItemsTable)
      .where(eq(salesInvoiceItemsTable.invoiceId, sibling.id));
    await db
      .delete(salesInvoicesTable)
      .where(eq(salesInvoicesTable.id, sibling.id));
  }
  return res.status(204).send();
});
...
router.post("/invoices/:id/cancel", requireAuth, async (req, res) => {
  try {
    ...
    if (invoice.status === "Cancelled")
      return res.json(serializeProforma(invoice));
    if (!["Approved", "Paid"].includes(invoice.status))
      return res
        .status(409)
        .json({ error: "Only approved or paid invoices can be cancelled" });
    const context = await accountingContext(req);
...
router.get("/payments", requireAuth, async (req, res) => {
  ...
  return res.json(
    rows.map((row) => ({
      ...
      bankCharges: Number(row.bankCharges),
      netReceived: Number(row.netReceived),
    })),
  );
});
...
router.post("/payments", requireAuth, async (req, res) => {
  try {
    ...
    const [receivable] = await db
      ...
      .limit(1);
    if (
      receivable?.sourceType === "Manual" &&
      receivable.approvalStatus !== "Approved"
    )
      return res.status(409).json({
        error:
          "The Accounts Receivable entry requires final approval before payment",
      });
    if (!(amount > 0))
      return res
        .status(400)
        .json({ error: "Payment amount must be greater than zero" });
    if (tdsAmount < 0 || bankCharges < 0 || tdsAmount + bankCharges > amount)
      return res.status(400).json({
        error:
          "TDS and bank charges must be non-negative and cannot exceed the payment amount",
      });
    if (amount > Number(invoice.balanceDue || invoice.grandTotal) + 0.009)
      return res.status(400).json({
        error: "Payment amount cannot exceed the invoice balance due",
      });
    const paymentDate = String(
      req.body.paymentDate || new Date().toISOString().slice(0, 10),
    );
    if (paymentDate < String(invoice.invoiceDate).slice(0, 10))
      return res.status(400).json({
        error: "Payment date cannot be earlier than the invoice date",
      });
    ...
    if (
      existingPayments.some(
        (row: any) =>
          Number(row.invoiceId) === invoiceId &&
          Number(row.amount) === amount &&
          String(row.paymentDate) === paymentDate &&
          String(row.reference || "") === reference,
      )
    )
      return res
        .status(409)
        .json({ error: "Duplicate payment submission detected" });
    ...
    const [updatedReceivable] = await db
      ...
      .where(
        and(
          eq(accountsReceivableTable.organizationId, context.organizationId),
          eq(accountsReceivableTable.sourceType, "Sales Invoice"),
          eq(accountsReceivableTable.sourceId, invoiceId),
        ),
      )
...
router.delete("/payments/:id", requireAuth, async (req, res) => {
  try {
    const context = await accountingContext(req);
    const deleted = await deletePaymentAccounting(
...
router.post("/receivable-adjustments", requireAuth, async (req, res) => {
  try {
    ...
    if (!["Approved", "Paid"].includes(invoice.status))
      return res.status(409).json({
        error: "Adjustments can be recorded only against approved invoices",
      });
    if (!(amount > 0))
      return res
        .status(400)
        .json({ error: "Adjustment amount must be greater than zero" });
    if (
      amount >
      Number(
      ...
      return res
        .status(400)
        .json({ error: "Adjustment cannot exceed the invoice balance" });
    const context = await accountingContext(req);
    const [adjustment] = await db
      .insert(salesReceivableAdjustmentsTable)
      .values({
        invoiceId,
      ...
      .returning();
    try {
      await triggerReceivableAdjustment(
        adjustment.id,
        context.organizationId,
        context.userId,
      );
...
router.delete("/receivable-adjustments/:id", requireAuth, async (req, res) => {
  try {
    const context = await accountingContext(req);
    const deleted = await deleteReceivableAdjustment(
      Number(req.params.id),
      context.organizationId,
    );
    if (!deleted)
      return res.status(404).json({ error: "Receivable adjustment not found" });
    return res.status(204).send();
  } catch (error: any) {
    return res.status(500).json({ error: error.message });
  }
...
function salesReturnData(payload: any) {
  return {
    ...
    dcId: payload.dcId ? Number(payload.dcId) : null,
    clientId: Number(payload.clientId),
    clientName: payload.clientName || "",
    customerMobile: payload.customerMobile || "",
    customerWhatsappNumber:
    ...
    customerGstin: payload.customerGstin || "",
    placeOfSupply: payload.placeOfSupply || "",
    returnDate: payload.returnDate || new Date().toISOString(),
    validUntil: payload.validUntil || null,
...
async function saveSalesReturnItems(returnId: number, items: any[]) {
  ...
  for (const item of items || []) {
    ...
    await db.insert(salesReturnItemsTable).values({
      ...
      description: item.description || item.productName || "",
      hsnSac: item.hsnSac || "",
      invoicedQty: String(item.invoicedQty ?? item.quantity ?? 0),
      returnedQty: String(item.returnedQty ?? item.quantity ?? 0),
      uom: item.uom || "Nos",
      rate: String(item.rate ?? 0),
      discountPercent: String(item.discountPercent ?? 0),
      cgstPercent: String(item.cgstPercent ?? 0),
      ...
      lineSource: item.lineSource || (service ? "Service" : "Inventory"),
      warehouseId: service ? null : (item.warehouseId ?? null),
      warehouseName: service ? "" : item.warehouseName || "",
      reason: item.reason || "",
...
async function validateInvoiceReturnAvailability(
  invoiceId: number,
  items: any[],
  ...
  const activeReturns = (await db.select().from(salesReturnsTable)).filter(
    (row) =>
      Number(row.invoiceId) === invoiceId &&
      Number(row.id) !== Number(excludeReturnId || 0) &&
      !["Cancelled", "Rejected"].includes(row.status),
  );
  ...
  for (const item of items) {
    ...
    const sourceItem = invoiceItems.find(
      (row) => Number(row.id) === invoiceItemId,
    );
    if (!sourceItem)
      throw new Error(
        `The source invoice item for ${item.description || "this line"} is invalid`,
      );
    ...
    if (requestedQty > remainingQty)
      throw new Error(
        `${item.description || sourceItem.description || "Item"} has ${priorQty} already returned; only ${remainingQty} remains returnable`,
      );
  }
}
...
async function salesReturnWithItems(doc: any) {
  const items = await db
    .select()
    .from(salesReturnItemsTable)
    .where(eq(salesReturnItemsTable.returnId, doc.id));
  return {
    ...serializeProforma(doc),
    items: items.map((item) => ({
      ...serializeQuotationItem(item),
...
async function restockSalesReturn(returnId: number, userId: number) {
  ...
  if (
    !doc ||
    doc.restocked ||
    !["Received", "Goods Received"].includes(doc.status) ||
    doc.restock === false
  )
    return;
  const items = await db
  ...
  for (const item of physical) {
    if (!Number(item.itemId || item.productId))
      throw new Error(`Inventory item is missing for ${item.description}`);
    if (!Number(item.warehouseId))
      throw new Error(
        `Receiving warehouse is required for ${item.description}`,
      );
  }
  const [claimed] = await db
    ...
    .where(
      and(
        eq(salesReturnsTable.id, returnId),
        eq(salesReturnsTable.restocked, false),
      ),
    )
    .returning();
  if (!claimed) return;
  ...
  try {
    for (const item of physical) {
      const materialId = Number(item.itemId || item.productId);
      const warehouseId = Number(item.warehouseId);
      const quantity = Number(item.returnedQty);
      ...
      if (!inventory)
        [inventory] = await db
          ...
          .values({ materialId, locationId: warehouseId, quantityOnHand: "0" })
          .returning();
      await db
        .update(inventoryTable)
        .set({
          quantityOnHand: String(oldQuantity + quantity),
          lastUpdated: new Date(),
        })
        .where(eq(inventoryTable.id, inventory.id));
      const [movement] = await db
        ...
        .values({
          ...
          quantityKg: String(quantity),
          reason: "Inward",
          notes: `Sales Return ${doc.returnNumber} (RETURN-${returnId})`,
          createdByUserId: userId,
        })
        .returning();
      completed.push({
    ...
    for (const entry of completed.reverse()) {
      await db
        ...
        .where(eq(inventoryMovementsTable.id, entry.movementId));
      if (entry.created)
        await db
          .delete(inventoryTable)
          .where(eq(inventoryTable.id, entry.inventoryId));
      else
        await db
          .update(inventoryTable)
          .set({
            quantityOnHand: String(entry.oldQuantity),
            lastUpdated: new Date(),
          })
          .where(eq(inventoryTable.id, entry.inventoryId));
    }
    await db
      .update(salesReturnsTable)
      .set({ restocked: false, restockedAt: null })
      .where(eq(salesReturnsTable.id, returnId));
    throw error;
  }
}
...
router.post("/returns", requireAuth, async (req, res) => {
  try {
    if (!req.body?.clientId)
      return res.status(400).json({ error: "Client is required" });
    if (req.body.invoiceId && req.body.dcId)
      return res.status(400).json({
        error: "Select either an Invoice or a Delivery Challan, not both",
      });
    const items = req.body.items || [];
    if (!items.length)
      return res
        .status(400)
        .json({ error: "At least one return item is required" });
    for (let index = 0; index < items.length; index++) {
      const item = items[index];
      const service =
        item.serviceId ||
        String(item.itemType || "").toLowerCase() === "service";
      if (Number(item.returnedQty ?? item.quantity) <= 0)
        return res.status(400).json({
          error: `Returned quantity must be greater than zero for line ${index + 1}`,
        });
      if (
        Number(item.returnedQty ?? item.quantity) >
        Number(item.invoicedQty ?? item.quantity)
      )
        return res.status(400).json({
          error: `Returned quantity cannot exceed source quantity for line ${index + 1}`,
        });
      if (!service && !item.warehouseId)
        return res.status(400).json({
          error: `Select the receiving warehouse for line item ${index + 1}`,
        });
    }
    if (req.body.invoiceId)
      await validateInvoiceReturnAvailability(
        Number(req.body.invoiceId),
        items,
      );
    const linkedInvoice =
      !req.body.invoiceId && req.body.dcId
        ? await linkedInvoiceForChallan(Number(req.body.dcId))
        : null;
    const returnNumber = returnCode(
      (await db.select().from(salesReturnsTable)).length + 1,
    );
    const [doc] = await db
      ...
      .values({
        ...
        ...salesReturnData({
          ...req.body,
          invoiceId: req.body.invoiceId || linkedInvoice?.id || null,
        }),
        status: "Draft",
        restocked: false,
      })
    ...
    await publishSalesCreatedNotification(req, res, {
      permissionKey: "sales.returns.notification",
      viewPermissionKey: "sales.returns.view",
      eventType: "SALES_RETURNS_DRAFT",
      title: "Sales return created",
      message: `${doc.returnNumber || `Sales return ${doc.id}`} was created for ${doc.clientName || "the customer"}.`,
      entityType: "sales_return",
      entityId: Number(doc.id),
      reference: doc.returnNumber || String(doc.id),
    });
...
router.get("/returns/:id", requireAuth, async (req, res) => {
  const [doc] = await db
    .select()
    .from(salesReturnsTable)
    .where(eq(salesReturnsTable.id, Number(req.params.id)))
    .limit(1);
  if (!doc) return res.status(404).json({ error: "Sales return not found" });
  return res.json(await salesReturnWithItems(doc));
});
...
router.patch("/returns/:id", requireAuth, async (req, res) => {
  ...
  if (
    ["Received", "Credit Issued", "Cancelled", "Rejected"].includes(
      existing.status,
    )
  )
    return res
      .status(409)
      .json({ error: "Received or closed Sales Returns cannot be edited" });
  if (
    Array.isArray(req.body.items) &&
    Number(req.body.invoiceId || existing.invoiceId)
  ) {
    try {
      await validateInvoiceReturnAvailability(
        Number(req.body.invoiceId || existing.invoiceId),
  ...
  const [doc] = await db
    .update(salesReturnsTable)
    .set({ ...salesReturnData({ ...existing, ...req.body }), status: "Draft" })
    .where(eq(salesReturnsTable.id, id))
    .returning();
  if (Array.isArray(req.body.items))
    await saveSalesReturnItems(id, req.body.items);
  return res.json(await salesReturnWithItems(doc));
});
...
router.post("/returns/:id/send", requireAuth, async (req, res) => {
  ...
  if (
    ...
    return res
      .status(409)
      .json({ error: "Received or closed Sales Returns cannot be sent" });
  if (Array.isArray(req.body.items))
    for (let index = 0; index < req.body.items.length; index++) {
      const item = req.body.items[index];
      const returned = Number(item.returnedQty ?? item.quantity);
      const source = Number(item.invoicedQty ?? item.quantity);
      const service =
        item.serviceId ||
        String(item.itemType || "").toLowerCase() === "service";
      if (!(returned > 0) || returned > source)
        return res
          .status(400)
          .json({ error: `Invalid returned quantity for line ${index + 1}` });
      if (!service && !item.warehouseId)
        return res.status(400).json({
          error: `Select the receiving warehouse for line item ${index + 1}`,
        });
  ...
  const [doc] = await db
    .update(salesReturnsTable)
    .set({
      ...salesReturnData({ ...existing, ...req.body }),
      status: "Confirmed",
    })
    .where(eq(salesReturnsTable.id, id))
    .returning();
...
router.post("/returns/:id/customer-response", requireAuth, async (req, res) => {
  ...
  if (
    ...
    !(existing.status === "Received" && action === "confirm")
  )
    return res.status(400).json({
      error: "Customer response is only allowed for confirmed Sales Returns",
    });
  if (!["confirm", "reject"].includes(action))
    return res.status(400).json({ error: "Invalid customer response" });
  ...
  if (status === "Received") {
    ...
    try {
      ...
      if (!existing.invoiceId && existing.dcId) {
        const linkedInvoice = await linkedInvoiceForChallan(
          Number(existing.dcId),
        );
        if (!linkedInvoice)
          throw new Error(
            "No approved or paid invoice is linked to this Delivery Challan",
          );
        await db
          .update(salesReturnsTable)
          .set({ invoiceId: linkedInvoice.id })
          .where(eq(salesReturnsTable.id, id));
      }
      await db
        .update(salesReturnsTable)
        .set({ status: "Credit Issued" })
        .where(eq(salesReturnsTable.id, id));
      await triggerSalesReturnCredited(
        id,
        context.organizationId,
        context.userId,
      );
      ...
      return res
        .status(500)
        .json({ error: `Sales return accounting failed: ${error.message}` });
    }
  }
  const [updated] = await db
    .select()
...
router.post("/returns/:id/issue-credit", requireAuth, async (req, res) => {
  try {
    ...
    if (!salesReturn)
      return res.status(404).json({ error: "Sales return not found" });
    if (!["Received", "Credit Issued", "Credited"].includes(salesReturn.status))
      return res.status(409).json({
        error: "Credit can be issued only after the return is received",
      });
    if (!salesReturn.invoiceId && salesReturn.dcId) {
      const linkedInvoice = await linkedInvoiceForChallan(
        Number(salesReturn.dcId),
      );
      if (!linkedInvoice)
        return res.status(409).json({
          error:
    ...
    return res
      .status(500)
      .json({ error: `Credit note accounting failed: ${error.message}` });
  }
});
...
router.delete("/returns/:id", requireAuth, async (req, res) => {
  ...
  const [doc] = await db
    ...
    .limit(1);
  if (!doc) return res.status(404).json({ error: "Sales return not found" });
  if (doc.restocked || ["Received", "Credit Issued"].includes(doc.status))
    return res
      .status(409)
      .json({ error: "Received Sales Returns cannot be deleted" });
  await db
    .delete(salesReturnItemsTable)
...
router.put("/organization", requireAuth, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err.message || "Failed to save organization details" });
  }
});
...
router.patch("/:id", requireAuth, async (req, res) => {
  ...
  await publishNotification({
    ...
    message: `${row.orderCode || `Sales order ${row.id}`} was updated.`,
    sourceEntityType: "sales_order",
    sourceEntityId: row.id,
    sourceReference: row.orderCode,
    navigationUrl: "/sales",
