...
router.post("/generate-sku", async (req, res) => {
  ...
  const [category] = await db
    ...
    .where(eq(inventoryCategoriesTable.id, Number(categoryId)));
  if (!category) {
  ...
  if (category.name) {
    const catStr = String(category.name)
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, "")
      .slice(0, 4);
...
router.post("/", async (req, res): Promise<any> => {
  ...
  try {
    const normalizedStocks = Array.isArray(warehouseStocks)
      ...
      : [];
    if (
      new Set(normalizedStocks.map((ws: any) => ws.warehouseId)).size !==
      normalizedStocks.length
    )
      return res
        .status(400)
        .json({ error: "Each warehouse can be selected only once" });
    for (const ws of normalizedStocks) {
      if (!Number.isFinite(ws.stock) || ws.stock < 0)
        return res
          .status(400)
          .json({ error: "Warehouse quantity must be zero or greater" });
      const [warehouse] = await db
        .select()
        .from(inventoryLocationsTable)
        .where(eq(inventoryLocationsTable.id, ws.warehouseId))
        .limit(1);
      if (!warehouse)
        return res
          .status(400)
          .json({ error: `Warehouse #${ws.warehouseId} was not found` });
    }
    const [mat] = await db
      ...
      .values({
        ...
        buyPricePerUnit:
          buyPricePerUnit != null ? String(buyPricePerUnit) : null,
        sellPricePerUnit:
    ...
    if (err.code === 11000 || err.code === "23505") {
      res
        .status(400)
        .json({ error: "Item with this Name or SKU already exists" });
    } else {
...
router.patch("/:id", async (req, res) => {
  ...
  if (buyPricePerUnit !== undefined)
    updates.buyPricePerUnit =
      buyPricePerUnit != null ? String(buyPricePerUnit) : null;
  if (sellPricePerUnit !== undefined)
    updates.sellPricePerUnit =
      sellPricePerUnit != null ? String(sellPricePerUnit) : null;
  ...
  if (defaultMoisturePercent !== undefined)
    updates.defaultMoisturePercent =
      defaultMoisturePercent != null ? String(defaultMoisturePercent) : null;
  if (defaultNitrogenPercent !== undefined)
    updates.defaultNitrogenPercent =
      defaultNitrogenPercent != null ? String(defaultNitrogenPercent) : null;
  ...
  if (Array.isArray(warehouseStocks)) {
    ...
    if (
      new Set(normalized.map((ws: any) => ws.warehouseId)).size !==
      normalized.length
    )
      return res
        .status(400)
    ...
    for (const ws of normalized) {
      const [warehouse] = await db
        ...
        .where(eq(inventoryLocationsTable.id, ws.warehouseId))
        .limit(1);
      if (!warehouse || !Number.isFinite(ws.stock) || ws.stock < 0)
        return res.status(400).json({ error: "Invalid warehouse or quantity" });
      const [existing] = await db
        .select()
        .from(inventoryTable)
        .where(
          and(
            eq(inventoryTable.materialId, mat.id),
            eq(inventoryTable.locationId, ws.warehouseId),
          ),
        )
        .limit(1);
      if (existing)
  ...
  return res.json({
    ...mat,
    defaultMoisturePercent:
      mat.defaultMoisturePercent != null
        ? Number(mat.defaultMoisturePercent)
        : null,
    defaultNitrogenPercent:
      mat.defaultNitrogenPercent != null
        ? Number(mat.defaultNitrogenPercent)
        : null,
  });
});
