...
const base = { mode: "Credit", bankCashAccountId: 1, amount: "100", transactionDate: "2026-09-07" };
...
test("explicit invalid client IDs cannot silently become an unassigned payment", () => {
  for (const clientId of [0, -1, false, "unknown"])
    assert.throws(() => prepareBankCash({ ...base, clientId }, accounts, clients), /Client/);
});
test("legacy Credit and Debit accept omitted optional fields", () => {
  for (const mode of ["Credit", "Debit"]) {
    const row = prepareBankCash({ ...base, mode }, accounts, clients);
    assert.equal(row.mode, mode);
    assert.equal(row.bankCharges, 0);
    assert.equal(row.transactionFees, 0);
    assert.equal(row.clientId, null);
    assert.equal(row.paymentMethod, "");
  }
});
test("Transfer resolves a different active destination", () => {
  assert.equal(prepareBankCash({ ...base, mode: "Transfer", counterAccount: "1002 - Other Bank" }, accounts, clients).transferToAccountId, 2);
  assert.throws(() => prepareBankCash({ ...base, mode: "Transfer", transferToAccountId: 1 }, accounts, clients), /different/);
  assert.throws(() => prepareBankCash({ ...base, mode: "Transfer" }, accounts, clients), /destination/);
});
test("dynamic COA and client selection, method, date and metadata", () => {
  ...
  const row = prepareBankCash({ ...base, bankCashAccountId: 12, clientName: "Client A - C007", paymentMode: "upi", notes: "One note", invoiceNumber: "INV-1", period: "Sep 2026", bankCharges: "2.50", transactionFees: "1" }, newAccounts, clients);
...
test("invalid accounts, clients, methods, modes and real calendar dates are rejected", () => {
  for (const patch of [{ bankCashAccountId: 99 }, { bankCashAccountId: 4 }, { clientId: 99 }, { paymentMethod: "Crypto" }, { mode: "Other" }, { transactionDate: "2026-02-30" }, { counterAccountId: 99 }])
    assert.throws(() => prepareBankCash({ ...base, ...patch }, accounts, clients));
  assert.throws(() => prepareBankCash({ ...base, clientName: "Client A" }, accounts, [...clients, { ...clients[0], id: 8 }]), /unambiguous/);
});
...
test("reordered new headers, aliases and missing optional columns", () => {
  for (const reference of ["Reference", "Reference ID", "Invoice Number", "Document Reference", "Reference ID / Invoice Number"]) {
    const rows = parseBankCashSheet([
      ["Notes", "Payment Date", "Account Name", reference, "Type", "Amount", "Payment Mode", "Client Name", "Bank Charges", "Transaction Fees", "Period"],
...
test("Excel export round-trips metadata with one Notes column and business reference", () => {
  const exported = bankCashExportRow({ ...base, accountDisplay: "1001 - Test Bank", clientDisplay: "Client A - C007", paymentMethod: "Cheque", reference: "INV-X", remarks: "Only note", bankCharges: 2, transactionFees: 3, period: "Q3" });
