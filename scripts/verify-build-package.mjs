...
function fail(message) { throw new Error(`Package verification failed: ${message}`); }
...
export function zipEntries(buffer) {
  ...
  if (eocd < 0) fail("ZIP end-of-central-directory record is missing or the archive is corrupt");
  const count = buffer.readUInt16LE(eocd + 10), offset = buffer.readUInt32LE(eocd + 16);
  const entries = []; let cursor = offset;
  for (let i = 0; i < count; i++) {
    if (buffer.readUInt32LE(cursor) !== 0x02014b50) fail(`invalid ZIP central-directory entry ${i + 1}`);
    ...
    entries.push(buffer.subarray(cursor + 46, cursor + 46 + nameLength).toString("utf8").replaceAll("\\", "/"));
...
export function verifyEntries(entries, kind, environment) {
  ...
  for (const name of files) {
    if (!name || name.startsWith("/") || /^[A-Za-z]:/.test(name) || name.split("/").includes("..")) fail(`unsafe archive path: ${name}`);
    if (/(^|[_ .-])copy([_ .-]|$)/i.test(path.basename(name)) || /_copy/i.test(name)) fail(`copy-suffixed file detected: ${name}; deploy into an empty target directory instead of renaming collisions`);
    const key = name.toLowerCase(); if (lower.has(key)) fail(`duplicate archive path (case-insensitive): ${name}`); lower.add(key);
  }
...
export async function verifyZip(zipFile, kind, environment) {
  ...
  if (!info?.isFile() || info.size < 22) fail(`ZIP does not exist or is empty: ${zipFile}`);
...
export async function verifyFrontendApi(frontendDirectory, expectedOrigin) {
  ...
  if (!found) fail(`compiled frontend does not contain configured API origin ${expectedOrigin}; API calls may return SPA HTML`);
}

export function verifyEcosystemText(text, environment) {
  const expected = `.env.${environment}`;
  if (!text.includes(`--env-file=${expected}`)) fail(`PM2 config does not explicitly load ${expected}`);
  for (const other of ["demo", "staging", "prod"].filter(value => value !== environment)) {
    if (text.includes(`--env-file=.env.${other}`)) fail(`PM2 config for ${environment} incorrectly references .env.${other}`);
  }
}
