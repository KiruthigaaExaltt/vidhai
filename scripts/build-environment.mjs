...
if (!["demo", "staging", "prod"].includes(environment)) {
  throw new Error(
    "Usage: node scripts/build-environment.mjs <demo|staging|prod> [--client-only|--server-only|--check]",
  );
}
...
async function requireFile(file, purpose) {
  if (!(await exists(file)))
    throw new Error(`${purpose} is missing: ${path.relative(root, file)}`);
}
...
async function frontendBuildConfig() {
  ...
  if (["demo", "staging", "prod"].includes(environment) && !corsRaw)
    throw new Error(`${envFileName}: backend CORS_ALLOWED_ORIGINS is required`);
  for (const origin of String(corsRaw || "")
    .split(",")
    ...
    try {
      ...
      throw new Error(`${envFileName}: invalid CORS origin ${origin}`);
    }
    if (
      !["http:", "https:"].includes(corsUrl.protocol) ||
      corsUrl.origin !== origin.replace(/\/$/, "")
    )
      throw new Error(`${envFileName}: CORS origins must not contain paths`);
  }
  ...
  try {
    ...
    throw new Error(
      `${envFileName}: VITE_API_BASE must be an absolute HTTP(S) URL`,
    );
  }
  if (
    ...
    throw new Error(
      `${envFileName}: VITE_API_BASE must contain only the API origin, without /api or another path`,
    );
...
async function nextPackage(prefix) {
  ...
  return {
    name,
    directory: path.join(outputRoot, name),
    zip: path.join(outputRoot, `${name}.zip`),
    number: highest + 1,
  };
}
...
async function packageFrontend() {
  ...
  if (frontendApiOrigin)
    await verifyFrontendApi(target.directory, frontendApiOrigin);
...
async function packageBackend() {
  ...
  if (await exists(sourceUploads))
    await cp(sourceUploads, packagedUploads, { recursive: true });
...
`;
  ...
  const seedCommand = `node --env-file=${envFileName} --enable-source-maps ./dist/seed-data.mjs`;
...
if (!serverOnly) {
  ...
  for (const name of [".htaccess", "web.config"]) {
    ...
    if (await exists(source))
      await copyFile(source, path.join(frontendDir, "dist", "public", name));
  }
  await packageFrontend();
}
if (!clientOnly) {
  await run(pnpm, ["--dir", "artifacts/api-server", "run", "build"], {
    env: { NODE_ENV: label },
  });
