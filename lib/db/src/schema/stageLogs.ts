...
export const stageLogsTable = mongoTable("stage_logs", {
  ...
  enteredByUserId: integer("entered_by_user_id").references(
    () => usersTable.id,
  ),
  completedByUserId: integer("completed_by_user_id").references(
    () => usersTable.id,
  ),
  chamberId: integer("chamber_id").references(() => chambersTable.id),
  chamberNameSnapshot: text("chamber_name_snapshot"),
  chamberTypeSnapshot: text("chamber_type_snapshot"),
  notes: text("notes"),
