...
export const insertOotyRoomSchema = createInsertSchema(ootyRoomsTable).omit({
  id: true,
});
...
export const ootyGrowingBatchesTable = mongoTable("ooty_growing_batches", {
  ...
  phaseEnteredAt: timestamp("phase_entered_at", {
    withTimezone: true,
  }).defaultNow(),
  ...
  substrateWeightKg: numeric("substrate_weight_kg", {
    precision: 10,
    scale: 4,
  }),
...
export const insertOotyGrowingBatchSchema = createInsertSchema(
  ootyGrowingBatchesTable,
).omit({ id: true, createdAt: true });
export type InsertOotyGrowingBatch = z.infer<
  typeof insertOotyGrowingBatchSchema
>;
...
export const ootyStageLogsTable = mongoTable("ooty_stage_logs", {
  ...
  casingSoilInventorySourceId: integer(
    "casing_soil_inventory_source_id",
  ).references(() => casingSoilInventorySourcesTable.id),
  casingSoilQuantityKg: numeric("casing_soil_quantity_kg", {
    precision: 12,
    scale: 4,
  }),
...
export const insertOotyStageLogSchema = createInsertSchema(
  ootyStageLogsTable,
).omit({ id: true });
...
export const ootyCasingRunConsumptionsTable = mongoTable(
  "ooty_casing_run_consumptions",
  {
    id: serial("id").primaryKey(),
    ...
    growingBatchId: integer("growing_batch_id")
      .notNull()
      .references(() => ootyGrowingBatchesTable.id, { onDelete: "cascade" }),
    stageLogId: integer("stage_log_id").references(() => ootyStageLogsTable.id),
    roomId: integer("room_id")
      .notNull()
      .references(() => ootyRoomsTable.id),
    inventorySourceId: integer("inventory_source_id")
      .notNull()
      .references(() => casingSoilInventorySourcesTable.id),
...
export const insertOotyBatchSourceSchema = createInsertSchema(
...
export const ootyGrowBagInventoryPostingsTable = mongoTable(
  "ooty_grow_bag_inventory_postings",
  {
    id: serial("id").primaryKey(),
    postingKey: text("posting_key").notNull().unique(),
    growingBatchId: integer("growing_batch_id")
...
export const insertOotyGrowBagInventoryPostingSchema = createInsertSchema(
  ootyGrowBagInventoryPostingsTable,
).omit({ id: true, createdAt: true });
export type OotyGrowBagInventoryPosting =
  typeof ootyGrowBagInventoryPostingsTable.$inferSelect;
...
export const insertOotyObservationSchema = createInsertSchema(
...
export const insertOotyHarvestSchema = createInsertSchema(
  ootyHarvestsTable,
).omit({ id: true, createdAt: true });
...
export const ootyHarvestInventoryPostingsTable = mongoTable(
  "ooty_harvest_inventory_postings",
  {
    id: serial("id").primaryKey(),
    postingKey: text("posting_key").notNull().unique(),
    growingBatchId: integer("growing_batch_id")
    ...
    harvestId: integer("harvest_id")
      .notNull()
      .references(() => ootyHarvestsTable.id, { onDelete: "cascade" }),
    flushNumber: integer("flush_number").notNull(),
    inventoryId: integer("inventory_id").notNull(),
    inventoryAdjustmentId: integer("inventory_adjustment_id").notNull(),
    warehouseId: integer("warehouse_id").notNull(),
    mushroomCount: integer("mushroom_count").notNull(),
    harvestWeightKg: numeric("harvest_weight_kg", {
      precision: 10,
      scale: 4,
    }).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true })
...
export const insertOotyHarvestInventoryPostingSchema = createInsertSchema(
  ootyHarvestInventoryPostingsTable,
).omit({ id: true, createdAt: true });
export type OotyHarvestInventoryPosting =
  typeof ootyHarvestInventoryPostingsTable.$inferSelect;

// One row per Cookout Manure stock posting. The unique key prevents retry/double-click duplication.
export const ootyCookoutInventoryPostingsTable = mongoTable(
  "ooty_cookout_inventory_postings",
  {
    id: serial("id").primaryKey(),
    postingKey: text("posting_key").notNull().unique(),
    growingBatchId: integer("growing_batch_id")
...
export const insertOotyCookoutInventoryPostingSchema = createInsertSchema(
  ootyCookoutInventoryPostingsTable,
).omit({ id: true, createdAt: true });
export type OotyCookoutInventoryPosting =
  typeof ootyCookoutInventoryPostingsTable.$inferSelect;
...
export const insertPhaseApprovalSchema = createInsertSchema(
  phaseApprovalsTable,
).omit({ id: true, createdAt: true });
